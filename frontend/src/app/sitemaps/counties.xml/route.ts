  ...
  if (SITEMAP_BATCH >= 4) {
    for (const county of counties) {
      ...
      for (const diag of diagnosesSlugs) {
        ...
        const rcScores = rcList.map((rc: RegionalCenter) => normalizeConfidenceScore(rc.confidence_score)).filter((s: number | null): s is number => s !== null);
        const sdScores = sdList.map((sd: SchoolDistrict) => normalizeConfidenceScore(sd.confidence_score)).filter((s: number | null): s is number => s !== null);
        const coScores = coList.map((co: CountyOffice) => normalizeConfidenceScore(co.confidence_score)).filter((s: number | null): s is number => s !== null);
        const allScores = [...rcScores, ...sdScores, ...coScores];
        const confScore = allScores.length > 0 ? allScores.reduce((sum, s) => sum + s, 0) / allScores.length : null;
