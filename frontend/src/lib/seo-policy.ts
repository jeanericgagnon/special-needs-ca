...
export function stateAuditStatus(stateId: string): { classification: string; indexSafe: boolean } | null {
  ...
  const stateObj = auditData.states.find((s: any) => s && s.stateId === normalized);
  const pqObj = priorityQueueData.find((s: any) => s && s.state === normalized);
  ...
  // Override Texas to not indexable
  if (normalized === 'texas') {
    return {
...
export function stateGapReason(stateId: string): string | null {
  ...
  const normalized = stateId.toLowerCase().trim();
  if (normalized === 'texas') {
    return 'Early Intervention Part C is marked verified but has sample_count: 0; county-local evidence still leans on DOI extract evidence. Treat as not safe for SEO indexation until repaired.';
  }
  const pqObj = priorityQueueData.find((s: any) => s && s.state === normalized);
  return pqObj ? pqObj.primary_gap_reason : null;
}
