...
test.describe('Eligibility Onboarding Wizard E2E Tests', () => {
  test('happy path: school-age autism in Los Angeles', async ({ page }) => {
    ...
    // Submit Step 3
    await page.click('button:has-text("Analyze observed needs")');
    ...
    // Answer refiners
    await page.click('button:has-text("Yes, active safety risks are present")');
    await page.click('button:has-text("Yes, we exceed income limits")');
    
    // Submit
    await page.click('button:has-text("Find Custom Matches")');
  ...
  test('conditional path: toddler speech delay in Orange County matches Early Start', async ({ page }) => {
    ...
    await page.click('button:has-text("Skip Answers")');
