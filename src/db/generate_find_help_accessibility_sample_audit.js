...
const jsonOutPath = path.join(docsDir, `find-help-accessibility-sample-audit-${generatedDate}.json`);
const mdOutPath = path.join(docsDir, `find-help-accessibility-sample-audit-${generatedDate}.md`);
...
function summarizeRow(row, languageField) {
  return {
    ...
    freshness: getFreshnessStamp(row) || null,
  };
}
...
function summarizeTable({ table, label, languageField }) {
  ...
  const selected = selectDiverseSamples(rows, languageField);
  const selectedAccessibilityCount = selected.filter((row) => hasDirectoryAccessibilitySignal(row, languageField)).length;
  const accessibleRows = rows.filter((row) => hasDirectoryAccessibilitySignal(row, languageField));
  const maxSelectedScore = selected.length ? Math.max(...selected.map((row) => getStructuredRichnessScore(row, languageField))) : 0;
  const equalOrBetterAccessibleRows = accessibleRows
    .filter((row) => getStructuredRichnessScore(row, languageField) >= maxSelectedScore)
    .slice(0, 25);
  const topAccessibleRows = [...accessibleRows]
    .sort(sortByCoverageThenFreshness(languageField))
    .slice(0, 10);
  ...
  return {
    ...
    accessibleCandidateCount: accessibleRows.length,
    selectedAccessibilityCount,
    selected,
    topAccessibleRows,
    equalOrBetterAccessibleRows,
  };
}
...
const mdLines = [
  '# Find Help Accessibility Sample Audit',
  '',
  `Generated: ${generatedDate}`,
  '',
  `DB audited: ${dbPath}`,
  '',
  'This audit checks whether the live `/find-help` samples are surfacing accessibility-aware rows when the trustworthy sample pool gives them that opportunity. It separates “the selector missed accessible rows” from “the dataset does not contain competitive accessible rows yet.”',
];

for (const table of tables) {
  mdLines.push('', `## ${table.label}`, '');
  mdLines.push(`- public candidates: ${table.publicCandidateCount}`);
  mdLines.push(`- accessible candidates: ${table.accessibleCandidateCount}`);
  mdLines.push(`- selected rows with accessibility signals: ${table.selectedAccessibilityCount}/${table.selected.length}`);

  mdLines.push('', 'Selected rows:', '');
  for (const row of table.selected.map((item) => summarizeRow(item, table.languageField))) {
    mdLines.push(`- ${row.id}: state=${row.state || 'unknown'}, family=${row.family}, score=${row.richnessScore}, accessibility=${row.hasAccessibility ? 'yes' : 'no'}, languages=${row.languageValue || 'none'}`);
  }

  if (table.topAccessibleRows.length) {
    mdLines.push('', 'Top accessible candidates:', '');
    for (const row of table.topAccessibleRows.map((item) => summarizeRow(item, table.languageField))) {
      mdLines.push(`- ${row.id}: state=${row.state || 'unknown'}, family=${row.family}, score=${row.richnessScore}, languages=${row.languageValue || 'none'}`);
    }
  } else {
    mdLines.push('', '- no accessible public candidates in this table');
  }

  if (table.equalOrBetterAccessibleRows.length) {
    mdLines.push('', 'Accessible rows at or above the selected score ceiling:', '');
    for (const row of table.equalOrBetterAccessibleRows.map((item) => summarizeRow(item, table.languageField)).slice(0, 10)) {
      mdLines.push(`- ${row.id}: state=${row.state || 'unknown'}, family=${row.family}, score=${row.richnessScore}, languages=${row.languageValue || 'none'}`);
    }
  }
}
