...
const jsonOutPath = path.join(docsDir, `competitive-help-followup-blocker-registry-${generatedDate}.json`);
const mdOutPath = path.join(docsDir, `competitive-help-followup-blocker-registry-${generatedDate}.md`);
const stateOutPath = path.join(stateDir, 'competitive-help-followup-blocker-registry.json');

const COMPETITIVE_FAMILIES = new Set([
  'housing',
  'goods_supplies',
  'jobs_vocational',
  ...
  for (const runId of fs.readdirSync(sourceAcquisitionRunsDir).sort()) {
    ...
    for (const [fileName, bucket] of [
      ...
      for (const row of rows) {
        if (!COMPETITIVE_FAMILIES.has(String(row.gapFamily || ''))) continue;
        ...
        if (!sourceUrl || !followupReason || !gapFamily) continue;
        const key = `${gapFamily}|${bucket}|${followupReason}|${sourceUrl}`;
        if (!grouped.has(key)) {
          grouped.set(key, {
            gapFamily,
            targetTable: row.targetTable || 'unknown',
            sourceUrl,
            hostname,
            bucket,
            followupReason,
...
const allRows = [...grouped.values()]
  ...
  .sort((a, b) =>
    a.gapFamily.localeCompare(b.gapFamily)
...
const payload = {
  ...
  summary: {
    ...
    byGapFamilyAll: countBy(allRows, 'gapFamily'),
    byGapFamily: countBy(rows, 'gapFamily'),
...
const mdLines = [
  '# Competitive Help Followup Blocker Registry',
  '',
  `Generated: ${payload.generatedAt}`,
  '',
  'This registry captures saved competitive-help followup failures that should be routed to source repair instead of being re-fetched in bounded lightweight waves.',
  '',
  '## Summary',
  '',
  `- total blocker rows: ${payload.summary.totalRows}`,
  ...
  '## By Gap Family',
  '',
  ...Object.entries(payload.summary.byGapFamily).sort((a, b) => b[1] - a[1]).map(([label, count]) => `- ${label}: ${count}`),
  '',
  '## By Bucket',
  '',
...
for (const row of rows) {
  mdLines.push(`- ${row.gapFamily} | ${row.bucket} | ${row.followupReason} | ${row.hostname} | repeats=${row.repeatCount} | ${row.sourceUrl}`);
}
