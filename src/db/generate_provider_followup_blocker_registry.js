...
const jsonOutPath = path.join(docsDir, `provider-followup-blocker-registry-${generatedDate}.json`);
const mdOutPath = path.join(docsDir, `provider-followup-blocker-registry-${generatedDate}.md`);
const stateOutPath = path.join(stateDir, 'provider-followup-blocker-registry.json');
      ...
      for (const row of rows) {
        ...
        if (!grouped.has(key)) {
          grouped.set(key, {
            ...
            targetTable: row.targetTable || 'resource_providers',
            sourceUrl,
            hostname,
            bucket,
            followupReason,
...
const mdLines = [
  ...
  '## Summary',
  '',
  `- total provider blocker rows: ${payload.summary.totalRows}`,
  `- repeated blocker rows: ${payload.summary.totalRepeatedRows}`,
...
console.log(JSON.stringify({
  ...
  markdown: mdOutPath,
  state: stateOutPath,
  totalRepeatedRows: payload.summary.totalRepeatedRows,
