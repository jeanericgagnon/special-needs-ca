  ...
  for (const prog of programs) {
    ...
    if (isEi) {
      ...
      insertRule.run(
        ...
        'any',
        'any',
        `Child is under age 3 and has active speech/language or communication needs; Early Intervention speech therapy assessments may be recommended in ${stateName}.`
      );
      insertRule.run(
        `rule-${pid}-feeding`,
        pid,
        ...
        'any',
        'any',
        `Child is under age 3 and has swallowing or feeding developmental delays; referral for occupational/physical therapy under ${name} may be recommended as a screening hint.`
      );
      ruleCount += 2;
      ...
      for (const cond of conditions) {
        if (cond.regional_center_relevance === 1 || cond.ccs_relevance === 1) {
          insertRule.run(
            ...
            'any',
            'any',
            `Child is under age 3 and has an established high-risk condition (${cond.name}); screening for Early Intervention services under ${name} in ${stateName} is recommended based on established high-risk conditions.`
          );
          ruleCount++;
        }
      }
      ...
      insertRule.run(
        ...
        'any',
        'any',
        `School-aged child exhibits speech, developmental, or academic needs suggesting a Special Education IEP evaluation under the IDEA Act in ${stateName} (screening hint).`
      );
      ruleCount++;
        ...
        for (const cond of conditions) {
          if (cond.regional_center_relevance === 1) {
            insertRule.run(
              ...
              'any',
              'any',
              `Diagnosed with ${cond.name} may qualify the child for the Florida Family Empowerment Scholarship for Students with Unique Abilities (FES-UA) to fund private schooling or therapies.`
            );
            ruleCount++;
          }
        }
      ...
      for (const cond of conditions) {
        if (cond.cal_able_relevance === 1) {
          insertRule.run(
            ...
            'any',
            'any',
            `Disability onset of ${cond.name} before age 26 may qualify for a tax-advantaged ${name} savings account.`
          );
          ruleCount++;
        }
      }
      ...
      for (const cond of conditions) {
        if (cond.regional_center_relevance === 1) {
          insertRule.run(
            ...
            'any',
            'any',
            `${cond.name} is a potential qualifying developmental disability category (screening hint) for the ${name} program in ${stateName}.`
          );
          ruleCount++;
        }
      }
      ...
      insertRule.run(
        ...
        'any',
        'any',
        `Respite hours may be available under ${name} to provide relief for family caregivers of children with developmental needs in ${stateName}.`
      );
      ruleCount += 2;
      ...
      insertRule.run(
        ...
        'any',
        'any',
        `Respite care hours may be available under ${name} to provide relief for family caregivers of children with developmental behaviors in ${stateName}.`
      );
      insertRule.run(
        `rule-${pid}-diapers`,
        pid,
      ...
      insertRule.run(
        ...
        'any',
        'any',
        `${stateName} Medicaid may provide comprehensive healthcare coverage, including EPSDT pediatric screenings, therapies, and medical equipment for eligible children with disabilities.`
      );
      // Diapers
      insertRule.run(
        `rule-${pid}-diapers`,
        ...
        'any',
        'any',
        `Medicaid may cover diapers and incontinence supplies starting at age 3 for children with developmental delays in ${stateName}.`
      );
      ruleCount += 2;
    } else if (isChip) {
      insertRule.run(
        ...
        'any',
        'any',
        `${stateName} Children's Health Insurance Program (CHIP) may provide low-cost health coverage for children in families that earn too much money to qualify for Medicaid.`
      );
      ruleCount++;
    } else if (isSsi) {
      for (const cond of conditions) {
        insertRule.run(
          ...
          'any',
          'any',
          `Assessments for ${cond.name} check for marked and severe functional limitations under childhood SSI guidelines in ${stateName} (screening hint).`
        );
        ruleCount++;
      }
      ...
      insertRule.run(
        ...
        'any',
        'any',
        `Transition services in ${stateName} may help young adults with disabilities transition from high school to employment or independent living.`
      );
      ruleCount++;
    } else if (isCcsLike) {
      for (const cond of conditions) {
        if (cond.ccs_relevance === 1) {
          insertRule.run(
            ...
            'any',
            'any',
            `${cond.name} may qualify the child for specialized pediatric medical and clinical therapy support under ${name} in ${stateName}.`
          );
          ruleCount++;
        }
      }
      ...
      if (pidLower.includes('childfind') || nameLower.includes('find')) {
        insertRule.run(
          ...
          'any',
          'any',
          `Child Find in ${stateName} helps identify, locate, and evaluate children from birth through age 21 who may need special education and related services (screening hint).`
        );
        ruleCount++;
      } else {
