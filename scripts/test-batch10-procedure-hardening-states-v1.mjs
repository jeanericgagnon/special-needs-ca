...
import { generateBatch10ProcedureHardeningStatesV1 } from './run-batch10-procedure-hardening-states-v1.mjs';
...
generateBatch10ProcedureHardeningStatesV1();

const batchSummary = readJson('data/generated/batch10_procedure_hardening_states_summary_v1.json');
...
const batchReport = fs.readFileSync(path.join(docsGeneratedDir, 'batch10-procedure-hardening-states-report-v1.md'), 'utf8');

assert.deepEqual(batchSummary.states.map((row) => row.state), ['north-carolina', 'south-carolina', 'idaho'], 'Batch 10 must cover North Carolina, South Carolina, and Idaho in order');
assert.equal(batchSummary.generated_all_state_v3, false, 'Batch 10 must not generate all-state v3 unless a state actually becomes complete');
assert.equal(batchSummary.complete_states.length, 0, 'No Batch 10 state should be promoted complete from current evidence');
assert.equal(batchSummary.texas_preserved_complete, true, 'Texas must remain COMPLETE/index_safe while Batch 10 runs');
...
for (const stateId of ['north-carolina', 'south-carolina', 'idaho']) {
...
assert.ok(batchReport.includes('generated_all_state_v3: false'), 'Batch 10 report must explicitly say all-state v3 was not generated');
assert.ok(batchReport.includes('texas_preserved_complete: true'), 'Batch 10 report must explicitly preserve Texas complete truth');
