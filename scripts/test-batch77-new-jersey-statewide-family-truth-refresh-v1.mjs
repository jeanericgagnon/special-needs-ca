...
import { generateBatch77NewJerseyStatewideFamilyTruthRefreshV1 } from './run-batch77-new-jersey-statewide-family-truth-refresh-v1.mjs';

const repoRoot = path.resolve(path.dirname(new URL(import.meta.url).pathname), '..');

const { summary, batchSummary } = generateBatch77NewJerseyStatewideFamilyTruthRefreshV1();
...
const verifiedRows = fs.readFileSync(path.join(repoRoot, 'data', 'generated', 'new-jersey_verified_sources_v1.jsonl'), 'utf8')
  .trim()
  .split('\n')
  .filter(Boolean)
...
assert.match(pti.samples[0].evidence_snippet, /NJ Statewide Parent to Parent/i);
assert.match(pti.samples[0].evidence_snippet, /Parent to Parent of NJ/i);
