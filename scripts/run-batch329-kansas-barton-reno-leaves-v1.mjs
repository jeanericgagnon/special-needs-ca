...
const OUTPUTS = {
  summary: path.join(generatedDir, 'batch329_kansas_barton_reno_leaves_summary_v1.json'),
  report: path.join(docsGeneratedDir, 'batch329-kansas-barton-reno-leaves-report-v1.md'),
};

const PRIMARY_GAP_REASON =
  'current_ksde_directory_roots_and_pdf_url_return_request_rejected_shells_and_exact_submit_replay_is_rejected_while_reviewed_local_district_leaves_cover_only_20_counties';
...
const FAMILY_STATUS =
  'blocked_reviewed_local_kansas_district_leaves_expand_to_20_counties_but_current_live_ksde_submit_replay_is_rejected';

const NEW_LEAVES = [
  {
    county_id: 'barton-ks',
    county_name: 'barton',
    district_name: 'Great Bend USD 428',
    district_website: 'https://www.greatbendschools.net/',
    source_url: 'https://sites.google.com/usd428.net/bartoncountyss/child-find',
    final_url: 'https://sites.google.com/usd428.net/bartoncountyss/child-find',
    verification_status: 'verified',
    source_type: 'district_linked_special_education_cooperative_child_find_leaf',
    fetched_at: '2026-06-24T00:00:00.000Z',
    evidence_title: 'BCSS - Child Find',
    evidence_h1: 'Child Find',
    evidence_snippet:
      'The official Great Bend USD 428 host links `BCSS - Barton County Special Services`, and the fetched district-linked child-find leaf preserves og:title `BCSS - Child Find` plus `BCSS staff engage in age-appropriate Child Find activities that locate, evaluate, and identify students who are in need of special education services`.',
  },
  {
    county_id: 'reno-ks',
    county_name: 'reno',
    district_name: 'Hutchinson Public Schools USD 308',
    district_website: 'https://www.usd308.com/',
    source_url: 'https://www.usd308.com/documents/resources/parent-resources/special-education/parental-rights/173274',
    final_url: 'https://www.usd308.com/documents/resources/parent-resources/special-education/parental-rights/173274',
    verification_status: 'verified',
    source_type: 'district_owned_special_education_parent_rights_document_folder',
    fetched_at: '2026-06-24T00:00:00.000Z',
    evidence_title: 'Documents | Hutchinson Public Schools',
    evidence_h1: 'Hutchinson Public Schools -',
    evidence_snippet:
      'The live district-owned Hutchinson Public Schools document route preserves breadcrumb context `Parent Resources` -> `Special Education/Parental Rights` and same-folder district files including `Parent_Rights.pdf` and `Spanish_-_Parental_Rights_PDF.pdf` on the official usd308.com host.',
  },
];
...
function buildEvidence(leaves) {
  ...
  return `Reviewed 2026-06-24 one more bounded official Kansas district-routing pass against the exact official KSDE roots plus one exact district-scoped submit replay, then promoted only exact local district-host evidence. The current live raw lane still returns the same \`Request Rejected\` shell for \`https://uapps.ksde.gov/Directory_Rpts/default.aspx\`, \`https://www.ksde.gov/data-and-reporting/directories\`, and \`https://www.ksde.gov/docs/default-source/crp/2025-2026-kansas-educational-directory.pdf?sfvrsn=7c81fd62_12\`. One fresh exact district-scoped submit replay against the current Directory Reports root using the public hidden fields and \`ctl00$MainContent$ddDistricts=D0435\`, \`ctl00$MainContent$RadioGroup1=RadioUSD1\`, \`ctl00$MainContent$rblFormat=Excel\`, and \`ctl00$MainContent$btnPrintSection1=Run Report\` also returned the same \`Request Rejected\` shell instead of a workbook. Kansas now has reviewed local proof for ${leaves.length}/105 counties from preserved district-owned or district-linked leaves: ${counties.join(', ')}. Barton now clears because the official Great Bend USD 428 host links \`BCSS - Barton County Special Services\` and the fetched district-linked child-find leaf states that BCSS staff locate, evaluate, and identify students who need special education services. Reno also now clears because the live Hutchinson Public Schools district-owned document route preserves breadcrumb context \`Special Education/Parental Rights\` and same-folder parent-rights files on the official usd308.com host. Kansas therefore remains blocked because county-grade local education proof is still incomplete across the remaining unresolved counties, and the only trustworthy next lane is saved district-owned or district-linked local leaf authoring rather than the current flapping-or-rejected KSDE export stack.`;
}
...
function buildStateReport(summary, gapRows, failureRows, verifiedRows, nextRows) {
  return [
    ...
    '- Barton now clears from the district-linked Barton County Special Services child-find leaf exposed from the official Great Bend USD 428 host.',
    '- Reno now clears from the district-owned Hutchinson Public Schools Special Education / Parental Rights document folder on the official usd308.com host.',
    '- Kansas now has reviewed local education-routing proof for twenty counties, but county-grade local coverage remains incomplete across the remaining unresolved counties.',
...
function buildBatchReport(batchSummary) {
  return [
    '# Batch 329 Kansas Barton And Reno Leaves v1',
    '',
...
export function generateBatch329KansasBartonRenoLeavesV1() {
  ...
  for (const leaf of NEW_LEAVES) {
    if (!leaves.some((row) => row.county_id === leaf.county_id && row.source_url === leaf.source_url)) {
      leaves.push(leaf);
    }
  }
  ...
  const updatedSummary = {
    ...summary,
    batch: 'batch329_kansas_barton_reno_leaves_v1',
    ...
    states: allStateAudit.states.map((row) => (
      row.stateId === 'kansas'
        ? {
            ...
            packetBatch: 'batch329_kansas_barton_reno_leaves_v1',
  ...
  const batchSummary = {
    ...
    new_counties: NEW_LEAVES.map((row) => row.county_id),
    next_action: NEXT_ACTION,
  };
