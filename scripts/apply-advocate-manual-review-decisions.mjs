...
import { blockerNote } from './advocate-manual-review-lib.mjs';
...
const defaultQueuePath = path.join(docsDir, `advocate-manual-review-queue-${generatedDate}.json`);
const defaultInputPath = path.join(dataDir, 'advocate-manual-review-decisions.json');
const ledgerPath = path.join(stateDir, 'advocate-manual-review-ledger.json');
...
if (!fs.existsSync(defaultQueuePath)) {
  throw new Error(`Missing advocate manual review queue: ${defaultQueuePath}. Run npm run audit:advocate-manual-review-queue first.`);
}
...
for (const row of selectedRows) {
  ...
  if (!['pending_review', 'needs_manual_review'].includes(String(liveRow.review_status || '').trim())) {
  ...
  if (args.apply) {
    ...
    const ledgerEntry = { ...normalizedRow, status: 'applied' };
...
const reportJsonPath = path.join(docsDir, `advocate-manual-review-decision-run-${runTimestamp}.json`);
const reportMdPath = path.join(docsDir, `advocate-manual-review-decision-run-${runTimestamp}.md`);
