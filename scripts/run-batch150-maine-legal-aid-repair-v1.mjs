...
const INPUTS = {
  ...
  legal: path.join(generatedDir, 'maine_legal_aid_reviewed_source_v1.json'),
};

const OUTPUTS = {
  summary: path.join(generatedDir, 'batch150_maine_legal_aid_repair_summary_v1.json'),
  report: path.join(docsGeneratedDir, 'batch150-maine-legal-aid-repair-report-v1.md'),
  ...
  return existingRows.map((row) => {
    if (row.family === 'legal_aid') {
      return {
        ...
        sample_count: 1,
        query_basis: 'Reviewed Pine Tree Legal Assistance homepage now provides direct statewide Maine legal-aid evidence.',
        blocker_code: null,
        blocker_evidence: null,
        samples: [
          {
            sample_name: 'Pine Tree Legal Assistance',
...
export function generateBatch150MaineLegalAidRepairV1() {
  ...
  assertIncludes(legalSource.page_title, 'Pine Tree Legal Assistance', 'Maine legal-aid reviewed source title');
  assertIncludes(legalSource.evidence_snippet, 'free civil legal aid in Maine', 'Maine legal-aid reviewed source snippet');
  ...
  const updatedGapRows = gapRows.map((row) => {
    if (row.family === 'legal_aid') {
      return {
        ...row,
        family_status: 'verified_state_grade',
        status_reason: 'Reviewed Pine Tree Legal Assistance now provides direct statewide Maine legal-aid evidence.',
      };
    }
    return row;
  });
    ...
    states: audit.states.map((row) => (
      row.stateId === 'maine'
        ? {
            ...
            packetBatch: 'batch_150_maine_legal_aid_repair_v1',
          }
        : row
    )),
  };
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const summary = generateBatch150MaineLegalAidRepairV1();
