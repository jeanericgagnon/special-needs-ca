...
import { generateBatch1FastFinishStatesV1 } from './run-batch1-fast-finish-states-v1.mjs';
...
generateBatch1FastFinishStatesV1();

const batchSummary = readJson('data/generated/batch1_fast_finish_states_summary_v1.json');
...
const batchReport = fs.readFileSync(path.join(docsGeneratedDir, 'batch1-fast-finish-states-report-v1.md'), 'utf8');

assert.deepEqual(batchSummary.states.map((row) => row.state), ['california', 'pennsylvania', 'florida'], 'Batch 1 must cover California, Pennsylvania, and Florida in order');
assert.equal(batchSummary.generated_all_state_v3, false, 'Batch 1 must not generate all-state v3 unless a state actually becomes complete');
assert.equal(batchSummary.complete_states.length, 0, 'No Batch 1 state should be promoted complete from current evidence');
assert.equal(batchSummary.texas_preserved_complete, true, 'Texas must remain COMPLETE/index_safe while Batch 1 runs');
...
for (const stateId of ['california', 'pennsylvania', 'florida']) {
...
assert.ok(batchReport.includes('generated_all_state_v3: false'), 'Batch report must explicitly say all-state v3 was not generated');
assert.ok(batchReport.includes('texas_preserved_complete: true'), 'Batch report must explicitly preserve Texas complete truth');
