...
import { generateBatch6ProcedureHardeningStatesV1 } from './run-batch6-procedure-hardening-states-v1.mjs';
...
generateBatch6ProcedureHardeningStatesV1();

const batchSummary = readJson('data/generated/batch6_procedure_hardening_states_summary_v1.json');
...
const batchReport = fs.readFileSync(path.join(docsGeneratedDir, 'batch6-procedure-hardening-states-report-v1.md'), 'utf8');

assert.deepEqual(batchSummary.states.map((row) => row.state), ['arkansas', 'oklahoma', 'wisconsin'], 'Batch 6 must cover Arkansas, Oklahoma, and Wisconsin in order');
assert.equal(batchSummary.generated_all_state_v3, false, 'Batch 6 must not generate all-state v3 unless a state actually becomes complete');
assert.equal(batchSummary.complete_states.length, 0, 'No Batch 6 state should be promoted complete from current evidence');
assert.equal(batchSummary.texas_preserved_complete, true, 'Texas must remain COMPLETE/index_safe while Batch 6 runs');
...
for (const stateId of ['arkansas', 'oklahoma', 'wisconsin']) {
...
assert.ok(batchReport.includes('generated_all_state_v3: false'), 'Batch 6 report must explicitly say all-state v3 was not generated');
assert.ok(batchReport.includes('texas_preserved_complete: true'), 'Batch 6 report must explicitly preserve Texas complete truth');
