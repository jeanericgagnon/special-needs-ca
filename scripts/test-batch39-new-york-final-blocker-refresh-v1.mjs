...
assert.equal(stateSummary.completeness_pct, 75, 'New York completeness should reflect the repaired statewide support families.');
...
assert.equal(legal.family_status, 'verified_state_grade', 'New York legal aid must remain verified from reviewed statewide LawHelpNY evidence.');
...
assert.ok(!failureRows.some((row) => row.family === 'legal_aid'), 'New York legal aid must no longer appear in the failure ledger.');
...
assert.deepEqual(
  ...
  'New York next actions must collapse to the three real remaining blockers.',
);
...
assert.equal(verifiedRows.find((row) => row.family === 'legal_aid').family_status, 'verified_state_grade', 'New York verified sources must preserve statewide legal aid.');
...
assert.ok(report.includes('ACCES-VR and Disability Rights New York remain upgraded out of the blocker list'), 'New York report must explain the reviewed statewide support upgrades.');
