...
const LIVE_PROBES = {
  ...
  desePartCGuess: {
    url: 'https://dese.mo.gov/childhood/first-steps',
    finalUrl: 'https://dese.mo.gov/childhood/first-steps',
    status: 404,
    contentType: 'text/html; charset=UTF-8',
    evidence: 'Reviewed 2026-06-21 bounded DESE repair pass did not surface a reviewed Part C / First Steps leaf, and the guessed DESE First Steps path returned HTTP 404.',
  },
};
...
function buildVerifiedSourceRows() {
  return [
    ...
    },
    {
      ...
      sample_count: 0,
      query_basis: 'The old dhhs.missouri.gov early-intervention packet root is dead and the bounded DESE repair pass did not surface a reviewed Part C / First Steps replacement leaf.',
      blocker_code: 'legacy_early_intervention_source_dead_and_no_reviewed_replacement',
      blocker_evidence: `${LIVE_PROBES.legacyEarlyInterventionRoot.evidence} ${LIVE_PROBES.desePartCGuess.evidence}`,
      samples: [],
    },
    {
      state: 'missouri',
    ...
    },
    {
      ...
      evidence_strength: 'missing',
      sample_count: 0,
      query_basis: 'The old packet incorrectly used Missouri DD pages for the VR / Pre-ETS family. The reviewed current DMH Employment Services leaf is still not a statewide VR or Pre-ETS routing source.',
      blocker_code: 'legacy_or_wrong_family_vr_sample',
      blocker_evidence: LIVE_PROBES.vrEmploymentServices.evidence,
      samples: [],
    },
    {
      state: 'missouri',
...
export function generateBatch45MissouriStatewideFamilyTruthRefreshV1() {
  ...
  const reasonByFamily = {
    ...
    early_intervention_part_c: 'The old dhhs.missouri.gov/earlystart packet root is dead and the bounded DESE repair pass did not surface a reviewed Part C / First Steps replacement leaf.',
    ...
    vocational_rehabilitation_pre_ets: 'The old packet used DD pages for this family and the reviewed DMH Employment Services leaf still does not prove statewide VR or Pre-ETS routing.',
  ...
  const updatedFailureRows = [
    {
      ...
      evidence: `${LIVE_PROBES.legacyEarlyInterventionRoot.evidence} ${LIVE_PROBES.desePartCGuess.evidence}`,
    ...
    },
    {
      ...
      evidence: LIVE_PROBES.vrEmploymentServices.evidence,
