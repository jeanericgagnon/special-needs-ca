...
const OUTPUTS = {
  summary: path.join(generatedDir, 'batch341_kansas_bourbon_fort_scott_leaf_summary_v1.json'),
  report: path.join(docsGeneratedDir, 'batch341-kansas-bourbon-fort-scott-leaf-report-v1.md'),
};

const COUNT = 28;
const PRIMARY_GAP_REASON =
  'current_ksde_directory_roots_and_pdf_url_return_request_rejected_shells_and_exact_submit_replay_is_rejected_while_reviewed_local_district_leaves_cover_only_28_counties';
const FAMILY_STATUS =
  'blocked_reviewed_local_kansas_district_leaves_expand_to_28_counties_but_current_live_ksde_submit_replay_is_rejected';
const NEXT_ACTION =
...
const BATCH = 'batch341_kansas_bourbon_fort_scott_leaf_v1';

const NEW_LEAF = {
  county_id: 'bourbon-ks',
  county_name: 'bourbon',
  district_name: 'Fort Scott USD 234',
  district_website: 'https://www.usd234.org/',
  source_url: 'https://www.usd234.org/page/special-education/',
  final_url: 'https://www.usd234.org/page/special-education/',
  verification_status: 'verified',
  source_type: 'district_owned_special_education_leaf',
  fetched_at: '2026-06-24T00:00:00.000Z',
  evidence_title: 'Special Education | FORT SCOTT USD 234',
  evidence_h1: null,
  evidence_snippet:
    'The official Fort Scott USD 234 sitemap exposes an exact same-domain `/page/special-education/` leaf, and the fetched district-owned page returned HTTP 200 with title `Special Education | FORT SCOTT USD 234` on the live usd234.org host.',
};
...
function buildStateReport(summary, gapRows, failureRows, verifiedRows, nextRows) {
  return [
    ...
    '- Bourbon now clears from the district-owned Fort Scott USD 234 `Special Education` leaf.',
...
function updateAllStateReport() {
  const note =
    '- Kansas remains blocked because the live KSDE Directory Reports root, Directories page, educational-directory PDF URL, and exact district-scoped submit replay still collapse to `Request Rejected` shells, even though reviewed local district-owned or district-linked special-education leaves now cover 28 of 105 counties, including the district-owned Fort Scott USD 234 `Special Education` leaf for Bourbon County.';
...
function buildBatchReport() {
  return [
    '# Batch 341 Kansas Bourbon Fort Scott Leaf v1',
    '',
    ...
    '',
    '- Bourbon now clears from the district-owned Fort Scott USD 234 `Special Education` leaf.',
    '- The official Fort Scott USD 234 sitemap exposes the exact `/page/special-education/` route on the district-owned host.',
    '- The fetched Fort Scott district leaf preserves the role-exact title `Special Education | FORT SCOTT USD 234` on the district host.',
...
export function generateBatch341KansasBourbonFortScottLeafV1() {
  ...
  const updatedPacket = {
    ...
    current_problem_metrics: {
      ...
      authoredExactLeafCount: 28,
      reviewedDistrictOwnedLeafCount: 28,
    },
    reviewed_local_leaf_counties: Array.from(new Set([...(packet.reviewed_local_leaf_counties || []), 'bourbon-ks'])).sort(),
    unresolved_local_leaf_counties: (packet.unresolved_local_leaf_counties || []).filter((countyId) => countyId !== 'bourbon-ks'),
  };
  ...
  const batchSummary = {
    ...
    classification: 'BLOCKED',
    index_safe: false,
    promoted_county: 'bourbon-ks',
    promoted_counties_total: 28,
    district_root_status: 200,
    district_root_title: 'Home | FORT SCOTT USD 234',
    district_leaf_status: 200,
    district_leaf_title: 'Special Education | FORT SCOTT USD 234',
