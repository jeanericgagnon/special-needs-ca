...
const OUTPUTS = {
  summary: path.join(generatedDir, 'batch338_kansas_coffey_ccsec_leaf_summary_v1.json'),
  report: path.join(docsGeneratedDir, 'batch338-kansas-coffey-ccsec-leaf-report-v1.md'),
};

const COUNT = 25;
const PRIMARY_GAP_REASON =
  'current_ksde_directory_roots_and_pdf_url_return_request_rejected_shells_and_exact_submit_replay_is_rejected_while_reviewed_local_district_leaves_cover_only_25_counties';
const FAMILY_STATUS =
  'blocked_reviewed_local_kansas_district_leaves_expand_to_25_counties_but_current_live_ksde_submit_replay_is_rejected';
const NEXT_ACTION =
...
const BATCH = 'batch338_kansas_coffey_ccsec_leaf_v1';

const NEW_LEAF = {
  county_id: 'coffey-ks',
  county_name: 'coffey',
  district_name: 'Burlington USD 244',
  district_website: 'https://www.usd244ks.org/',
  source_url: 'https://www.usd244ks.org/ccsec',
  final_url: 'https://www.usd244ks.org/ccsec',
  verification_status: 'verified',
  source_type: 'district_linked_special_education_cooperative_home',
  fetched_at: '2026-06-24T00:00:00.000Z',
  evidence_title: 'Burlington USD 244 - CCSEC Home',
  evidence_h1: 'Burlington USD #244',
  evidence_snippet:
    'The official Burlington USD 244 homepage exposes a district-linked `Coffey County Special Education Cooperative` route, and the fetched `usd244ks.org/ccsec` page preserves the county-named cooperative identity plus a statement that CCSEC provides a broad continuum of services for students from birth through 21 years of age in USD 244 Burlington and USD 245 Southern Coffey County.',
};
...
function buildBatchReport() {
  return [
    '# Batch 338 Kansas Coffey CCSEC Leaf v1',
    '',
    ...
    '',
    '- Coffey now clears from the district-linked Burlington USD 244 `Coffey County Special Education Cooperative` route.',
    '- The official Burlington USD 244 homepage exposes the exact `Coffey County Special Education Cooperative` route on the district-owned host.',
    '- The fetched CCSEC page preserves the county-named cooperative identity and says it provides a broad continuum of services for students from birth through 21 years of age in USD 244 Burlington and USD 245 Southern Coffey County.',
...
export function generateBatch338KansasCoffeyCcsecLeafV1() {
  ...
  const updatedPacket = {
    ...
    current_problem_metrics: {
      ...
      authoredExactLeafCount: 25,
      reviewedDistrictOwnedLeafCount: 25,
    },
    reviewed_local_leaf_counties: Array.from(new Set([...(packet.reviewed_local_leaf_counties || []), 'coffey-ks'])).sort(),
    unresolved_local_leaf_counties: (packet.unresolved_local_leaf_counties || []).filter((countyId) => countyId !== 'coffey-ks'),
  };
  ...
  const batchSummary = {
    ...
    classification: 'BLOCKED',
    index_safe: false,
    promoted_county: 'coffey-ks',
    promoted_counties_total: 25,
    district_root_status: 200,
    district_root_title: 'Burlington USD 244 - Home',
    district_leaf_status: 200,
    district_leaf_title: 'Burlington USD 244 - CCSEC Home',
    district_leaf_h1: 'Burlington USD #244',
  };
