...
const BATCH_STATES = ['tennessee', 'virginia', 'indiana'];
const STATE_CODE = { tennessee: 'TN', virginia: 'VA', indiana: 'IN' };
const STATE_LABEL = { tennessee: 'Tennessee', virginia: 'Virginia', indiana: 'Indiana' };
...
function buildStateSummary(stateRow, stateFailures, priorityRow, verifiedSources) {
  ...
  return {
    ...
    batch: 'batch_8_procedure_hardening_states_v1',
...
function buildStateReport(stateRow, summary, gapRows, failureRows, verifiedSources, nextActions) {
  return [
    `# ${stateRow.stateName} California-Grade Batch 8 Report v1`,
    '',
...
function buildBatchReport(batchSummary) {
  return [
    '# Batch 8 Procedure Hardening States Report v1',
    '',
    'This pass creates state-specific California-grade repair control-plane artifacts for Tennessee, Virginia, and Indiana using the hardened Texas v10 rules. It does not promote any state without full critical-family verification.',
    '',
    '## State status',
    '',
    ...
    '',
    '- No new reusable lesson was promoted from Batch 8; existing Texas v10 rules and the current lessons docs remain authoritative for the next batch.',
  ].join('\n');
}

export function generateBatch8ProcedureHardeningStatesV1() {
  ...
  try {
    ...
    writeJson(path.join(generatedDir, 'batch8_procedure_hardening_states_summary_v1.json'), batchSummary);
    fs.writeFileSync(path.join(docsGeneratedDir, 'batch8-procedure-hardening-states-report-v1.md'), `${buildBatchReport(batchSummary)}\n`);
    return batchSummary;
  } finally {
    db.close();
  }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const summary = generateBatch8ProcedureHardeningStatesV1();
