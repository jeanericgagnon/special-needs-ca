...
const OUTPUTS = {
  summary: path.join(generatedDir, 'batch285_alaska_legacy_dhss_subtree_finality_summary_v1.json'),
  report: path.join(docsGeneratedDir, 'batch285-alaska-legacy-dhss-subtree-finality-report-v1.md'),
};

const PRIMARY_GAP_REASON = 'live_dfcs_services_page_only_provides_statewide_phone_relay_while_current_health_and_legacy_dhss_dpa_hosts_stay_gate_blocked';
const FAILURE_CODE = 'live_dfcs_services_page_is_phone_only_while_current_health_host_and_legacy_dhss_dpa_subtree_both_fail_closed';
const FAMILY_STATUS = 'blocked_phone_only_dfcs_relay_plus_current_health_host_403_and_legacy_dhss_dpa_subtree_403';
const NEXT_ACTION = 'hold_blocked_until_alaska_publishes_borough_or_census_area_to_dpa_office_mapping_on_a_reviewable_official_surface_or_reopens_a_reviewable_dpa_directory_host';
...
const LESSON_HEADING = '### A Partially Live Legacy Root Does Not Reopen A Gated Subtree';
const LESSON_BODY =
...
function buildAllStateReport(allStateAudit) {
  ...
  return [
    ...
    '- Alaska county-local routing is now explicitly sharpened to a phone-only DFCS relay plus a fully challenged current health host and a partially live but gated legacy DHSS DPA subtree.',
  ].join('\n') + '\n';
}

function buildBatchReport(summary, lessonsUpdated) {
  return [
    '# Batch 285 Alaska Legacy DHSS Subtree Finality Report v1',
    '',
    '- state: Alaska',
    `- classification: ${summary.classification}`,
    ...
    '- No current or legacy official DPA host exposes a reviewable county-equivalent office contract in the low-token lane.',
  ].join('\n') + '\n';
}

export function generateBatch285AlaskaLegacyDhssSubtreeFinalityV1() {
  ...
  const updatedVerifiedRows = verifiedRows.map((row) => {
    ...
    const samples = [
      ...keep,
      {
        ...
        source_table: 'batch285_alaska_legacy_dhss_subtree_finality',
    ...
    return {
      ...
      query_basis: 'Reviewed 2026-06-23 the live DFCS Services host, the current health host family, and the legacy DHSS root plus exact DPA subtree paths.',
  ...
  const batchSummary = {
    batch: 'batch285_alaska_legacy_dhss_subtree_finality_v1',
    ...
    blocker_family: 'county_local_disability_resources',
    failure_code: FAILURE_CODE,
    legacy_dhss_root_live_but_subtree_gated: true,
