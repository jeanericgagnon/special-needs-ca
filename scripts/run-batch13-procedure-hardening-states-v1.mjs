...
const BATCH_STATES = ['alaska', 'maine', 'nevada'];
const STATE_CODE = { alaska: 'AK', maine: 'ME', nevada: 'NV' };
const STATE_LABEL = { alaska: 'Alaska', maine: 'Maine', nevada: 'Nevada' };
...
function buildStateSummary(stateRow, stateFailures, priorityRow, verifiedSources) {
  ...
  return {
    ...
    batch: 'batch_13_procedure_hardening_states_v1',
...
function buildStateReport(stateRow, summary, gapRows, failureRows, verifiedSources, nextActions) {
  return [
    `# ${stateRow.stateName} California-Grade Batch 13 Report v1`,
    '',
...
function buildBatchReport(batchSummary) {
  return [
    '# Batch 13 Procedure Hardening States Report v1',
    '',
    'This pass creates state-specific California-grade repair control-plane artifacts for Alaska, Maine, and Nevada using the hardened Texas v10 rules. It does not promote any state without full critical-family verification.',
    '',
    '## State status',
    '',
    ...
    '',
    '- No new reusable lesson was promoted from Batch 13; existing Texas v10 rules and the current lessons docs remain authoritative for the next batch.',
  ].join('\n');
}

export function generateBatch13ProcedureHardeningStatesV1() {
  ...
  try {
    ...
    writeJson(path.join(generatedDir, 'batch13_procedure_hardening_states_summary_v1.json'), batchSummary);
    fs.writeFileSync(path.join(docsGeneratedDir, 'batch13-procedure-hardening-states-report-v1.md'), `${buildBatchReport(batchSummary)}\n`);
    return batchSummary;
  } finally {
    db.close();
  }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const summary = generateBatch13ProcedureHardeningStatesV1();
