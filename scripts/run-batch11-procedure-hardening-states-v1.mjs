...
const BATCH_STATES = ['oregon', 'colorado', 'michigan'];
const STATE_CODE = { oregon: 'OR', colorado: 'CO', michigan: 'MI' };
const STATE_LABEL = { oregon: 'Oregon', colorado: 'Colorado', michigan: 'Michigan' };
...
function buildStateSummary(stateRow, stateFailures, priorityRow, verifiedSources) {
  ...
  return {
    ...
    batch: 'batch_11_procedure_hardening_states_v1',
...
function buildStateReport(stateRow, summary, gapRows, failureRows, verifiedSources, nextActions) {
  return [
    `# ${stateRow.stateName} California-Grade Batch 11 Report v1`,
    '',
...
function buildBatchReport(batchSummary) {
  return [
    '# Batch 11 Procedure Hardening States Report v1',
    '',
    'This pass creates state-specific California-grade repair control-plane artifacts for Oregon, Colorado, and Michigan using the hardened Texas v10 rules. It does not promote any state without full critical-family verification.',
    '',
    '## State status',
    '',
    ...
    '',
    '- No new reusable lesson was promoted from Batch 11; existing Texas v10 rules and the current lessons docs remain authoritative for the next batch.',
  ].join('\n');
}

export function generateBatch11ProcedureHardeningStatesV1() {
  ...
  try {
    ...
    writeJson(path.join(generatedDir, 'batch11_procedure_hardening_states_summary_v1.json'), batchSummary);
    fs.writeFileSync(path.join(docsGeneratedDir, 'batch11-procedure-hardening-states-report-v1.md'), `${buildBatchReport(batchSummary)}\n`);
    return batchSummary;
  } finally {
    db.close();
  }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const summary = generateBatch11ProcedureHardeningStatesV1();
