...
const BATCH_STATES = ['georgia', 'ohio', 'new-york'];
const STATE_CODE = { georgia: 'GA', ohio: 'OH', 'new-york': 'NY' };
const STATE_LABEL = { georgia: 'Georgia', ohio: 'Ohio', 'new-york': 'New York' };
...
function buildStateSummary(stateRow, stateFailures, priorityRow, verifiedSources) {
  ...
  return {
    ...
    batch: 'batch_2_repair_blocked_states_v1',
...
function buildStateReport(stateRow, summary, gapRows, failureRows, verifiedSources, nextActions) {
  return [
    `# ${stateRow.stateName} California-Grade Batch 2 Report v1`,
    '',
...
function buildBatchReport(batchSummary) {
  return [
    '# Batch 2 Repair Blocked States Report v1',
    '',
    'This pass creates state-specific California-grade repair control-plane artifacts for Georgia, Ohio, and New York using the hardened Texas v10 rules. It does not promote any state without full critical-family verification.',
    '',
    '## State status',
    '',
    ...
    '',
    '- No new reusable lesson was promoted from Batch 2; existing Texas v10 rules and the current lessons docs remain authoritative for the next batch.',
  ].join('\n');
}

export function generateBatch2RepairBlockedStatesV1() {
  ...
  try {
    ...
    writeJson(path.join(generatedDir, 'batch2_repair_blocked_states_summary_v1.json'), batchSummary);
    fs.writeFileSync(path.join(docsGeneratedDir, 'batch2-repair-blocked-states-report-v1.md'), `${buildBatchReport(batchSummary)}\n`);
    return batchSummary;
  } finally {
    db.close();
  }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const summary = generateBatch2RepairBlockedStatesV1();
