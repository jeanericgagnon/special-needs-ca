...
const OUTPUTS = {
  summary: path.join(generatedDir, 'batch313_kansas_rejected_shell_submit_finality_summary_v1.json'),
  report: path.join(docsGeneratedDir, 'batch313-kansas-rejected-shell-submit-finality-report-v1.md'),
};

const PRIMARY_GAP_REASON =
  'official_ksde_directory_and_pdf_roots_now_only_serve_request_rejected_shells_and_the_directory_reports_root_exposes_no_hidden_submit_fields_while_reviewed_district_owned_leaves_cover_16_counties';
const FAILURE_CODE =
  'official_ksde_directory_and_pdf_roots_now_only_serve_request_rejected_shells_and_the_directory_reports_root_exposes_no_hidden_submit_fields_while_reviewed_district_owned_leaves_cover_16_counties';
const NEXT_ACTION =
...
const FAMILY_STATUS =
  'blocked_reviewed_district_owned_and_coop_leads_but_live_ksde_roots_now_only_expose_rejected_shell_without_submit_contract';
const STATUS_REASON =
...
const LESSON_HEADING = '### A Rejected ASP.NET Shell Without Hidden Fields Means The Public Submit Contract Is Gone';
const LESSON_BODY =
...
export function generateBatch313KansasRejectedShellSubmitFinalityV1() {
  ...
  const updatedSummary = {
    ...summary,
    batch: 'batch313_kansas_rejected_shell_submit_finality_v1',
  ...
  const updatedVerifiedRows = verifiedRows.map((row) => (
    row.family === 'district_or_county_education_routing'
      ? {
          ...
          blocker_evidence: 'The current KSDE Directory Reports root, Directories root, and educational-directory PDF URL all return the same `Request Rejected` shell in the bounded raw lane, and the Directory Reports shell exposes no `__VIEWSTATE`, `__VIEWSTATEGENERATOR`, or `__EVENTVALIDATION` fields.',
          query_basis: 'Reviewed the current official KSDE state roots plus one exact district-scoped submit attempt in the bounded raw lane, then preserved the district-grade blocker against the existing reviewed district-leaf inventory.',
        }
      : row
  ));
  ...
  const updatedNextRows = nextRows.map((row) => (
    row.family === 'district_or_county_education_routing'
      ? {
          ...
          evidence: 'The live KSDE roots now return a shared `Request Rejected` shell and the Directory Reports shell exposes no hidden ASP.NET submit fields, so the remaining work must stay on saved district leads plus exact district-owned leaves.',
        }
      : row
  ));
    ...
    states: allStateAudit.states.map((row) => (
      row.stateId === 'kansas'
        ? {
            ...
            packetBatch: 'batch313_kansas_rejected_shell_submit_finality_v1',
  ...
  const batchSummary = {
    ...
    rejected_shell: true,
    has_viewstate: false,
    has_viewstategenerator: false,
    has_eventvalidation: false,
