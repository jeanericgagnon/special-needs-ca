...
assert.equal(summary.primary_gap_reason, 'three_public_district_domains_sitemap_exhausted_and_des_county_lane_still_lacks_county_office_contract');
...
assert.equal(countyGap.family_status, 'blocked_des_challenge_plus_single_yuma_altcs_html_without_county_contract');
assert.match(countyGap.status_reason, /single visible local office card for Yuma/i);
...
assert.equal(countyFailure.failure_code, 'des_roots_still_challenged_and_ahcccs_altcs_html_only_exposes_single_yuma_office');
assert.match(countyFailure.evidence, /Yuma ALTCS Office/i);
...
assert.equal(countyVerified.blocker_code, 'des_roots_still_challenged_and_ahcccs_altcs_html_only_exposes_single_yuma_office');
...
assert.equal(nextRows.find((row) => row.family === 'county_local_disability_resources').next_action, 'hold_blocked_until_des_clears_or_ahcccs_publishes_county_to_office_contract');

assert.equal(educationPacket.current_problem_metrics.reviewedPublicDomainSitemapExhaustedCount, 3);
assert.equal(countyPacket.current_problem_metrics.altcsRawHtmlVisibleOfficeCount, 1);
assert.equal(queueRows.find((row) => row.state === 'arizona').primary_gap_reason, 'three_public_district_domains_sitemap_exhausted_and_des_county_lane_still_lacks_county_office_contract');
assert.equal(batchSummary.unresolved_public_district_domains, 3);
assert.equal(batchSummary.ahcccs_visible_local_offices_in_raw_html, 1);
assert.match(report, /single visible Yuma ALTCS office/i);
