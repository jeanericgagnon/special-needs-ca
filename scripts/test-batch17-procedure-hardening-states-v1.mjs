...
import { generateBatch17ProcedureHardeningStatesV1 } from './run-batch17-procedure-hardening-states-v1.mjs';
...
generateBatch17ProcedureHardeningStatesV1();

const batchSummary = readJson('data/generated/batch17_procedure_hardening_states_summary_v1.json');
...
const batchReport = fs.readFileSync(path.join(docsGeneratedDir, 'batch17-procedure-hardening-states-report-v1.md'), 'utf8');

assert.deepEqual(batchSummary.states.map((row) => row.state), ['new-jersey'], 'Batch 17 must cover only New Jersey');
assert.equal(batchSummary.generated_all_state_v3, false, 'Batch 17 must not generate all-state v3 unless a state actually becomes complete');
assert.equal(batchSummary.complete_states.length, 0, 'Batch 17 must not promote New Jersey complete from current evidence');
assert.equal(batchSummary.texas_preserved_complete, true, 'Texas must remain COMPLETE/index_safe while Batch 17 runs');
...
for (const stateId of ['new-jersey']) {
...
assert.ok(batchReport.includes('generated_all_state_v3: false'), 'Batch 17 report must explicitly say all-state v3 was not generated');
assert.ok(batchReport.includes('texas_preserved_complete: true'), 'Batch 17 report must explicitly preserve Texas complete truth');
