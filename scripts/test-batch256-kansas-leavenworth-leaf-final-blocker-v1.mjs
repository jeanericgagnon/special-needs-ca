...
assert.equal(summary.primary_gap_reason, 'reviewed_kansas_district_owned_leaves_now_cover_9_counties_but_export_backed_county_grade_coverage_is_still_incomplete');
...
assert.match(gap.status_reason, /9\/105 counties/i);
...
assert.equal(failure.failure_code, 'reviewed_kansas_district_owned_leaves_now_cover_9_counties_but_export_backed_county_grade_coverage_is_still_incomplete');
assert.match(failure.evidence, /usd453\.org\/district-departments\/special-education/i);
assert.match(failure.evidence, /usd383\.org\/32689_3/i);
...
assert.ok(verified.samples.some((sample) => sample.sample_name === 'riley district-owned leaf'));
assert.ok(verified.samples.some((sample) => sample.sample_name === 'saline district-linked cooperative lead'));
assert.ok(verified.samples.some((sample) => sample.sample_name === 'riley district early-learning special-education support leaf'));
...
assert.equal(queue.primary_gap_reason, 'reviewed_kansas_district_owned_leaves_now_cover_9_counties_but_export_backed_county_grade_coverage_is_still_incomplete');

assert.equal(packet.current_problem_metrics.reviewedDistrictOwnedLeafCount, 9);
assert.ok(packet.reviewed_local_leaf_counties.includes('leavenworth-ks'));
assert.ok(packet.reviewed_local_leaf_counties.includes('riley-ks'));
...
assert.ok(leaves.some((row) => row.county_id === 'riley-ks' && row.source_url === 'https://www.usd383.org/32689_3'));

assert.equal(batchSummary.newly_verified_county, 'leavenworth-ks');
assert.equal(batchSummary.newly_verified_supporting_county, 'riley-ks');
assert.equal(batchSummary.saline_coop_lead_preserved, true);
assert.equal(batchSummary.riley_verified_leaf_added, true);
assert.ok(report.includes('Riley now clears from exact district-owned USD 383 special-education leaves, raising the reviewed county total to nine.'));
