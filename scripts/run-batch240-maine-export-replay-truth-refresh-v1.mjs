...
const OUTPUTS = {
  batchSummary: path.join(generatedDir, 'batch240_maine_export_replay_truth_refresh_summary_v1.json'),
  batchReport: path.join(docsGeneratedDir, 'batch240-maine-export-replay-truth-refresh-report-v1.md'),
  stateReport: path.join(docsGeneratedDir, 'maine-california-grade-audit-report-v2.md'),
};

const PRIMARY_GAP_REASON = 'public_maine_sau_selectors_and_workbook_are_live_but_export_replay_still_500_and_dhhs_office_html_has_no_county_contract';
const EDUCATION_FAILURE_CODE = 'live_public_sau_selectors_and_workbook_exist_but_export_replay_still_500';
const EDUCATION_NEXT_ACTION = 'preserve_manual_export_or_browser_capture_lane_and_do_not_treat_raw_export_as_recovered';
...
const LESSON_HEADING = '### Revalidate “Recovered” Postback Lanes With The Literal Submit Value';
const LESSON_BODY = '*   **Lesson:** If an earlier state note says a postback lane recovered, rerun the exact named submit with its literal current button value before promoting the lane. Maine NEO still exposed `action:SAUExport`, but a fresh Bangor `OrgId=42` replay with `Export to Excel` continued to return HTTP 500 shell HTML.';
...
export function generateBatch240MaineExportReplayTruthRefreshV1() {
  ...
  const updatedGapRows = gapRows.map((row) => (
    row.family === 'district_or_county_education_routing'
      ? { ...row, family_status: 'blocked_live_public_sau_selectors_and_workbook_but_export_replay_still_500', status_reason: EDUCATION_STATUS_REASON }
      : row
  ));
  ...
  const updatedVerifiedRows = verifiedRows.map((row) => (
    row.family === 'district_or_county_education_routing'
      ? {
          ...row,
          family_status: 'blocked_live_public_sau_selectors_and_workbook_but_export_replay_still_500',
          query_basis: 'Reviewed the live official Maine Org selector HTML, Town selector, workbook, and a fresh bounded raw export replay that still ends in HTTP 500 shell HTML.',
          ...
          samples: [
            {
              ...
              evidence_snippet: 'The public selector HTML itself remains live and exposes the `OrgId` selector plus the named submit actions for Search and Export.',
            },
            {
            ...
            },
            {
              ...
              fetched_at: '2026-06-23T00:00:00.000Z',
              evidence_snippet: 'A fresh Bangor `OrgId=42` replay with the literal `action:SAUExport=Export to Excel` value still returned HTTP 500 and only the generic NEO shell HTML rather than `SAUSearchResults.xls` or local contact rows.',
            },
            {
              ...
              fetched_at: '2026-06-23T00:00:00.000Z',
              evidence_snippet: 'The public inventory lane exists, but the current raw export replay still has not yielded reusable county-grade local routing rows across all 16 Maine counties.',
            },
          ],
        }
      : row
  ...
  const updatedPacket = {
    ...
    packet_complete_when: 'Every Maine county row either points at reviewed local SAU or district contact evidence or remains explicitly blocked because the live public selectors and municipality workbook are public but the bounded raw export replay still returns HTTP 500 shell HTML.',
  };
  ...
  const batchSummary = {
    batch: 'batch_240_maine_export_replay_truth_refresh_v1',
