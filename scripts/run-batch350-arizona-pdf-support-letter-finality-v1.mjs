...
const LESSON_BODY =
  '*   **Lesson:** When a blocker says the PDF lane is unavailable, re-check the exact official PDFs once the bundled runtime can parse or render them, and also confirm that the live download path has not shifted underneath a still-live oversight page. Arizona\'s AHCCCS UniversityFamilyCare page now points to working `/Resources/Downloads/UniversityFamilyCare/` PDFs while the older stale `/Resources/OversightOfHealthPlans/*.pdf` paths replay to HTML shells. Even after correcting the live file path, the files were still non-contract support letters rather than county-routing evidence.';
...
export function generateBatch350ArizonaPdfSupportLetterFinalityV1() {
        ...
        samples: [
          {
            ...
            evidence_snippet: 'The public page stays live and links `Pima Community Access Program`, `Pima County Administrator\'s Office`, and `County Administrator\'s Office`, but the current live file contract is the `Resources/Downloads/UniversityFamilyCare/` path while the older stale `OversightOfHealthPlans/*.pdf` URLs now replay to the AHCCCS HTML shell.',
          },
          {
  ...
  const batchSummary = {
    ...
    live_download_path_corrected: true,
    stale_pdf_paths_return_html_shell: true,
