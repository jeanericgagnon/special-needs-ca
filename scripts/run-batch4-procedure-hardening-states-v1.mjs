...
const BATCH_STATES = ['missouri', 'kansas', 'nebraska'];
const STATE_CODE = { missouri: 'MO', kansas: 'KS', nebraska: 'NE' };
const STATE_LABEL = { missouri: 'Missouri', kansas: 'Kansas', nebraska: 'Nebraska' };
...
function buildStateSummary(stateRow, stateFailures, priorityRow, verifiedSources) {
  ...
  return {
    ...
    batch: 'batch_4_procedure_hardening_states_v1',
...
function buildStateReport(stateRow, summary, gapRows, failureRows, verifiedSources, nextActions) {
  return [
    `# ${stateRow.stateName} California-Grade Batch 4 Report v1`,
    '',
...
function buildBatchReport(batchSummary) {
  return [
    '# Batch 4 Procedure Hardening States Report v1',
    '',
    'This pass creates state-specific California-grade repair control-plane artifacts for Missouri, Kansas, and Nebraska using the hardened Texas v10 rules. It does not promote any state without full critical-family verification.',
    '',
    '## State status',
    '',
    ...
    '',
    '- No new reusable lesson was promoted from Batch 4; existing Texas v10 rules and the current lessons docs remain authoritative for the next batch.',
  ].join('\n');
}

export function generateBatch4ProcedureHardeningStatesV1() {
  ...
  try {
    ...
    writeJson(path.join(generatedDir, 'batch4_procedure_hardening_states_summary_v1.json'), batchSummary);
    fs.writeFileSync(path.join(docsGeneratedDir, 'batch4-procedure-hardening-states-report-v1.md'), `${buildBatchReport(batchSummary)}\n`);
    return batchSummary;
  } finally {
    db.close();
  }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const summary = generateBatch4ProcedureHardeningStatesV1();
