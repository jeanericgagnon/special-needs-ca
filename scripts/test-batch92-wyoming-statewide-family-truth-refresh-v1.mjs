...
import { generateBatch92WyomingStatewideFamilyTruthRefreshV1 } from './run-batch92-wyoming-statewide-family-truth-refresh-v1.mjs';
...
const result = generateBatch92WyomingStatewideFamilyTruthRefreshV1();
assert.equal(result.summary.classification, 'BLOCKED');
assert.equal(result.summary.index_safe, false);

const summary = readJson(path.join(repoRoot, 'data', 'generated', 'wyoming_california_grade_summary_v2.json'));
...
const gapRows = readJsonl(path.join(repoRoot, 'data', 'generated', 'wyoming_gap_matrix_v2.jsonl'));
assert.equal(
...
const verifiedRows = readJsonl(path.join(repoRoot, 'data', 'generated', 'wyoming_verified_sources_v1.jsonl'));
...
assert.match(pandaRow.samples[0].evidence_snippet, /Protection and Advocacy System/);
...
assert.match(report, /Wyoming is therefore terminal BLOCKED, not COMPLETE\./);
assert.match(report, /Protection and Advocacy System, Inc\./);
