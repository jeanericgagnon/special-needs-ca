...
const OUTPUTS = {
  summary: path.join(generatedDir, 'batch293_alaska_dfcs_search_finality_summary_v1.json'),
  report: path.join(docsGeneratedDir, 'batch293-alaska-dfcs-search-finality-report-v1.md'),
};

const PRIMARY_GAP_REASON =
  'live_dfcs_services_page_and_public_dfcs_search_still_expose_no_borough_routing_while_current_health_and_legacy_dhss_dpa_hosts_stay_gate_blocked';
const FAILURE_CODE =
  'live_dfcs_services_page_is_phone_only_and_public_dfcs_search_is_generic_while_current_health_host_and_legacy_dhss_dpa_subtree_both_fail_closed';
const FAMILY_STATUS =
  'blocked_phone_only_dfcs_relay_plus_generic_dfcs_search_plus_current_health_host_403_and_legacy_dhss_dpa_subtree_403';
const NEXT_ACTION =
...
const STATUS_REASON =
  'The live Alaska DFCS Services page is still only a statewide phone relay, and one more bounded pass now proves the DFCS host itself still adds no public borough- or census-area office contract. `https://dfcs.alaska.gov/Search/default.aspx` is publicly readable, but the query replays stay on the same generic SharePoint search shell and expose no reviewable DPA or office results contract; the page form also only posts back to `./default.aspx` and exposes no public `k` or `terms` query field. The current `health.alaska.gov` family still returns the same Cloudflare 403 shell on exact office leaves, service/resource roots, `robots.txt`, `sitemap.xml`, `wp-json`, and `wp-sitemap.xml`. The legacy `dhss.alaska.gov` root and `robots.txt` remain publicly readable, but the exact DPA subtree is also not reviewable: `/dpa/Pages/default.aspx`, `/dpa/Pages/office-locations.aspx`, `/dpa/Pages/contacts.aspx`, and `/dpa/Pages/Publications.aspx` all return HTTP 403, while `sitemap.xml` and SharePoint search routes return 404. That means Alaska still lacks any public borough- or census-area office-routing contract on current DFCS, current health, or legacy DHSS official hosts.';

const EVIDENCE =
  'Reviewed 2026-06-23 bounded official Alaska rechecks across the live DFCS successor host, the DFCS public search surface, the current `health.alaska.gov` family, and the legacy `dhss.alaska.gov` DPA subtree. `https://dfcs.alaska.gov/Pages/Services.aspx` remains live and publicly reviewable, but still provides only statewide phone routing for `Adult Public Assistance` and `Apply for Medicaid` through `888-804-6330` with no borough or census-area mapping. `https://dfcs.alaska.gov/Search/default.aspx` is also publicly readable, but bounded query replays such as `?query=public+assistance`, `?query=medicaid`, and `?query=office+locations` all stay on the same generic SharePoint search shell without exposing any reviewable DPA office, office-locations, borough, or census-area result rows; the page form still only posts back to `./default.aspx` and exposes no public `k` or `terms` query field. The current `health.alaska.gov` family still fails closed end to end: exact office and service leaves such as `/en/resources/division-of-public-assistance-dpa-offices/`, `/en/services/adult-public-assistance-apa/`, and `/en/services/division-of-public-assistance-services/apply-for-medicaid/` return HTTP 403, and the same 403 applies to `robots.txt`, `sitemap.xml`, `wp-json`, `wp-sitemap.xml`, and the parent `/en/resources/` and `/en/services/` roots. The legacy `dhss.alaska.gov` host is only partially public: the root and `robots.txt` return 200, but `sitemap.xml` and SharePoint search routes return 404, and the exact DPA subtree is still not reviewable because `/dpa/Pages/default.aspx`, `/dpa/Pages/office-locations.aspx`, `/dpa/Pages/contacts.aspx`, `/dpa/Pages/Publications.aspx`, and `/dsds/Pages/default.aspx` all return HTTP 403. Alaska therefore still lacks any reviewable borough- or census-area-to-office contract on a public official surface, and the blocker is now sharper because even the live DFCS search surface does not open a usable DPA results lane.';

const LESSON_HEADING = '### A Public SharePoint Search Shell Is Not A Directory Contract';
const LESSON_BODY =
...
function buildStateReport(summary, gapRows, failureRows, verifiedRows, nextRows) {
  return [
    ...
    '- The public DFCS search shell is readable, but bounded query replays still expose no reviewable DPA office results or public query contract.',
...
export function generateBatch293AlaskaDfcsSearchFinalityV1() {
  ...
  const updatedVerifiedRows = verifiedRows.map((row) => {
    ...
    const samples = [
      ...keep,
      {
        ...
        source_type: 'official_search_shell_without_result_contract',
        source_table: 'batch293_alaska_dfcs_search_finality',
        fetched_at: '2026-06-23T00:00:00.000Z',
        evidence_snippet: 'The public DFCS search page is readable, but the page still renders only a generic SharePoint search shell and does not expose a public DPA office result contract.',
      },
      {
        sample_name: 'DFCS Search query replay',
        source_url: 'https://dfcs.alaska.gov/Search/default.aspx?query=public+assistance',
        final_url: 'https://dfcs.alaska.gov/Search/default.aspx?query=public+assistance',
        verification_status: 'blocked',
        source_type: 'official_search_query_without_server_results',
        source_table: 'batch293_alaska_dfcs_search_finality',
        fetched_at: '2026-06-23T00:00:00.000Z',
        evidence_snippet: 'Bounded query replays such as `?query=public+assistance`, `?query=medicaid`, and `?query=office+locations` all stay on the same generic search shell and expose no public DPA office rows.',
      },
      {
    ...
    return {
      ...
      query_basis: 'Reviewed 2026-06-23 the live DFCS Services host, the public DFCS search shell, the current health host family, and the legacy DHSS root plus exact DPA subtree paths.',
  ...
  const batchSummary = {
    batch: 'batch293_alaska_dfcs_search_finality_v1',
    ...
    blocker_family: 'county_local_disability_resources',
    failure_code: FAILURE_CODE,
    dfcs_search_public_but_generic: true,
