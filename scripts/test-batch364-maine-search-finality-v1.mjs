...
assert.match(countyGap.status_reason, /generic search shell/i);
...
assert.match(countyFailure.evidence, /maine\.gov\/search/i);
assert.match(countyFailure.evidence, /generic portal shell/i);
...
assert.equal(queueRow.primary_gap_reason, 'official_dhhs_nav_stack_and_official_maine_search_still_expose_office_addresses_and_labels_but_no_county_or_service_area_contract');

const auditRow = allStateAudit.states.find((row) => row.stateId === 'maine');
assert.equal(auditRow.packetBatch, 'batch364_maine_search_finality_v1');
assert.equal(auditRow.packetPrimaryGapReason, 'official_dhhs_nav_stack_and_official_maine_search_still_expose_office_addresses_and_labels_but_no_county_or_service_area_contract');
assert.equal(auditRow.familyStatuses.county_local_disability_resources, 'blocked_public_dhhs_nav_stack_and_state_search_without_county_to_office_or_service_area_assignment_contract');
...
assert.match(allStateReport, /official-search finality check/i);
assert.match(handoff, /Current Focus State: Maine/);
assert.match(handoff, /public `maine\.gov\/search` query pages stay live/i);
assert.match(handoff, /Official Maine Search: county district office ofi/);
assert.match(handoff, /1\. Idaho/);
assert.match(lessons, /Official State Search Shells Are Still Negative Evidence When They Materialize No Role Results/);
...
assert.match(batchReport, /official Maine search host/i);
