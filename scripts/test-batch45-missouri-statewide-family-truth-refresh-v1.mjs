...
assert.equal(summary.completeness_pct, 58, 'Missouri completeness must reflect seven strong critical families after truth refresh.');
assert.equal(summary.strong_critical_families, 7, 'Missouri should retain seven strong critical families after the truth refresh.');
assert.equal(summary.weak_critical_families, 2, 'Missouri should retain two weak critical families after the truth refresh.');
assert.equal(summary.missing_critical_families, 3, 'Missouri should retain three missing critical families after the truth refresh.');
...
assert.equal(failureRows.find((row) => row.family === 'vocational_rehabilitation_pre_ets').failure_code, 'legacy_or_wrong_family_vr_sample');
...
assert.equal(verifiedByFamily.get('early_intervention_part_c').sample_count, 0, 'Missouri early intervention must not preserve stale samples.');
assert.equal(verifiedByFamily.get('vocational_rehabilitation_pre_ets').sample_count, 0, 'Missouri VR / Pre-ETS must clear the wrong-family DD sample.');
