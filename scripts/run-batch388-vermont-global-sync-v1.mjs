...
const OUTPUTS = {
  summary: path.join(generatedDir, 'batch388_vermont_global_sync_summary_v1.json'),
  report: path.join(docsGeneratedDir, 'batch388-vermont-global-sync-report-v1.md'),
};
...
function buildAllStateReport(currentText, updatedAudit) {
  ...
  const vtNote = '- Vermont is now COMPLETE/index-safe from the current official AHS Field Services ArcGIS map and REST layers plus the reviewed Vermont government school-to-district open-data dataset.';
...
function buildBatchReport(updatedAudit) {
  return [
    '# Batch 388 Vermont Global Sync v1',
    '',
    ...
    '## Change',
    '',
    '- Integrated the reviewed Vermont completion into the global audit, priority queue, all-state report, and Gemini handoff.',
    '- Preserved the current focus state section while moving Vermont from BLOCKED to COMPLETE/index-safe in the statewide control plane.',
  ].join('\n') + '\n';
}

export function generateBatch388VermontGlobalSyncV1() {
  ...
  const batchSummary = {
    batch: 'batch388_vermont_global_sync_v1',
    generated_at: new Date().toISOString(),
    vermont_classification: 'COMPLETE',
