...
import { generateBatch14ProcedureHardeningStatesV1 } from './run-batch14-procedure-hardening-states-v1.mjs';
...
generateBatch14ProcedureHardeningStatesV1();

const batchSummary = readJson('data/generated/batch14_procedure_hardening_states_summary_v1.json');
...
const batchReport = fs.readFileSync(path.join(docsGeneratedDir, 'batch14-procedure-hardening-states-report-v1.md'), 'utf8');

assert.deepEqual(batchSummary.states.map((row) => row.state), ['delaware', 'vermont', 'wyoming'], 'Batch 14 must cover Delaware, Vermont, and Wyoming in order');
assert.equal(batchSummary.generated_all_state_v3, false, 'Batch 14 must not generate all-state v3 unless a state actually becomes complete');
assert.equal(batchSummary.complete_states.length, 0, 'No Batch 14 state should be promoted complete from current evidence');
assert.equal(batchSummary.texas_preserved_complete, true, 'Texas must remain COMPLETE/index_safe while Batch 14 runs');
...
for (const stateId of ['delaware', 'vermont', 'wyoming']) {
...
assert.ok(batchReport.includes('generated_all_state_v3: false'), 'Batch 14 report must explicitly say all-state v3 was not generated');
assert.ok(batchReport.includes('texas_preserved_complete: true'), 'Batch 14 report must explicitly preserve Texas complete truth');
