...
const INPUTS = {
  wisconsinSummary: path.join(generatedDir, 'wisconsin_california_grade_summary_v2.json'),
  wisconsinGap: path.join(generatedDir, 'wisconsin_gap_matrix_v2.jsonl'),
...
const OUTPUTS = {
  summary: path.join(generatedDir, 'batch380_wisconsin_global_sync_summary_v1.json'),
  report: path.join(docsGeneratedDir, 'batch380-wisconsin-global-sync-report-v1.md'),
};
...
function buildAllStateReport(currentText, updatedAudit) {
  ...
  const wisconsinNote = '- Wisconsin is now COMPLETE/index-safe from the live DPI School Directory county/CESA routing contract plus refreshed official DHS developmental-disability, Birth to 3, and DVR transition evidence.';
  const lines = currentText
    .split('\n')
    .filter((line) => !line.startsWith('- Wisconsin is now COMPLETE/index-safe'));
...
function buildHandoff(currentText, updatedAudit) {
  ...
  const blockedHeading = lines.findIndex((line) => line.trim() === '## Current Blocked States');
...
function buildBatchReport(updatedAudit) {
  return [
    '# Batch 380 Wisconsin Global Sync v1',
    '',
    ...
    '## Change',
    '',
    '- Integrated the safe Wisconsin-only parallel-state commit into the global audit, priority queue, all-state report, and Gemini handoff.',
    '- Preserved New Hampshire as the current focus state while moving Wisconsin from BLOCKED to COMPLETE/index-safe in the statewide control plane.',
  ].join('\n') + '\n';
}

export function generateBatch380WisconsinGlobalSyncV1() {
  const wisconsinSummary = readJson(INPUTS.wisconsinSummary);
  const wisconsinGapRows = readJsonl(INPUTS.wisconsinGap);
  const queueRows = readJsonl(INPUTS.queue);
  const allStateAudit = readJson(INPUTS.audit);
  const allStateReport = fs.readFileSync(INPUTS.allStateReport, 'utf8');
  const handoff = fs.readFileSync(INPUTS.handoff, 'utf8');

  const wisconsinFamilyStatuses = Object.fromEntries(
    wisconsinGapRows.map((row) => [row.family, row.family_status])
  );
  ...
  const updatedStates = allStateAudit.states.map((row) => (
    row.stateId === 'wisconsin'
      ? {
          ...row,
          classification: 'COMPLETE',
          indexSafe: true,
          incorrectlyIndexSafe: false,
          strongCriticalFamilies: wisconsinSummary.strong_critical_families,
          weakCriticalFamilies: wisconsinSummary.weak_critical_families,
          missingCriticalFamilies: wisconsinSummary.missing_critical_families,
          totalCriticalFamilies: wisconsinSummary.total_critical_families,
          completenessPct: wisconsinSummary.completeness_pct,
          familyStatuses: wisconsinFamilyStatuses,
          packetGenerated: true,
          packetBatch: wisconsinSummary.batch,
          packetPrimaryGapReason: wisconsinSummary.primary_gap_reason,
          packetRecommendedBatch: wisconsinSummary.recommended_batch,
        }
      : row
  ));
  ...
  const batchSummary = {
    batch: 'batch380_wisconsin_global_sync_v1',
    generated_at: new Date().toISOString(),
    wisconsin_classification: 'COMPLETE',
