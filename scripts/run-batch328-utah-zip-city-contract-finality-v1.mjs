...
const OUTPUTS = {
  batchSummary: path.join(generatedDir, 'batch328_utah_zip_city_contract_finality_summary_v1.json'),
  batchReport: path.join(docsGeneratedDir, 'batch328-utah-zip-city-contract-finality-report-v1.md'),
};
...
function buildStateReport(summary, gapRows, failureRows, verifiedRows, nextRows) {
  return [
    ...
    '- The surviving official DWS office-search shell now makes the public lookup contract even narrower by explicitly limiting the footer search input to `Zip Code or City`.',
...
function updateAllStateReport() {
  ...
  if (!current.includes(note)) {
    const insertAfter = '- Nebraska remains blocked because the public ExperienceBuilder config itself still references only one web map plus a closest-feature output and geocoder search, while the underlying FeatureServer still exposes no county-assignment bridge beyond the same 42-office / 93-county mismatch.';
    if (current.includes(insertAfter)) {
      current = current.replace(insertAfter, `${insertAfter}\n${note}`);
    } else {
...
export function generateBatch328UtahZipCityContractFinalityV1() {
  ...
  const updatedSummary = {
    ...summary,
    batch: 'batch328_utah_zip_city_contract_finality_v1',
  ...
  const updatedVerifiedRows = verifiedRows.map((row) => (
    row.family === 'county_local_disability_resources'
      ? {
          ...
          blocker_evidence: 'The recovered DHHS contacts page explicitly defers local office information, the live DWS shell only supports zip-or-city lookup, and the surviving public API surface still stops at office inventory plus three service labels without any county or service-area contract.',
          query_basis: 'Reviewed the current live DHHS contacts page, the DWS contact page, office-search shell, footer search contract, public offices API, public services API, and bounded same-host county-style endpoint probes.',
          samples: [
            {
              ...
              source_table: 'batch328_utah_zip_city_contract_finality',
  ...
  const updatedNextRows = nextRows.map((row) => (
    row.family === 'county_local_disability_resources'
      ? {
          ...
          evidence: 'The recovered DHHS contacts page explicitly defers local office information, the live DWS shell only supports zip-or-city lookup, and the surviving DWS public API surface still stops at inventory plus the three service labels `All`, `USOR`, and `EC` with no county contract.',
        }
      : row
  ));
    ...
    states: allStateAudit.states.map((row) => (
      row.stateId === 'utah'
        ? {
            ...
            packetBatch: 'batch328_utah_zip_city_contract_finality_v1',
