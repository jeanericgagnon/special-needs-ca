...
const INPUTS = {
  evidence: path.join(generatedDir, 'virginia_independent_reaudit_evidence_v1.json'),
...
const OUTPUTS = {
  summary: path.join(generatedDir, 'batch374_virginia_independent_reaudit_summary_v1.json'),
  report: path.join(docsGeneratedDir, 'batch374-virginia-independent-reaudit-report-v1.md'),
...
function buildReport(summary, gapRows, verifiedRows, evidenceRows) {
  ...
  return [
    ...
    '',
    ...evidenceRows
      .filter((row) => row.review_mode === 'browser_reviewed')
      .map((row) => {
        const sample = row.samples[0];
        return `- ${row.family}: ${sample.source_url} :: ${sample.evidence_snippet}`;
      }),
    '',
    '## Failure ledger',
    '',
...
export function generateBatch374VirginiaIndependentReauditV1() {
  ...
  const updatedSummary = {
    ...
    major_gap_families: [],
    primary_gap_reason: 'all_critical_families_independently_reaudited_with_live_official_or_browser_reviewed_official_evidence',
    complete_ready: true,
    final_blockers: null,
  };

  const batchSummary = {
    batch: 'batch374_virginia_independent_reaudit_v1',
    state: 'virginia',
    classification: updatedSummary.classification,
    index_safe: updatedSummary.index_safe,
    reviewed_critical_families: evidenceRows.filter((row) => row.critical).map((row) => row.family),
    browser_reviewed_families: evidenceRows.filter((row) => row.review_mode === 'browser_reviewed').map((row) => row.family),
  };
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const result = generateBatch374VirginiaIndependentReauditV1();
