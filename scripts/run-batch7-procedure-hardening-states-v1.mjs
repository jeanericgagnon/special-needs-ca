...
const BATCH_STATES = ['alabama', 'south-dakota', 'louisiana'];
const STATE_CODE = { alabama: 'AL', 'south-dakota': 'SD', louisiana: 'LA' };
const STATE_LABEL = { alabama: 'Alabama', 'south-dakota': 'South Dakota', louisiana: 'Louisiana' };
...
function buildStateSummary(stateRow, stateFailures, priorityRow, verifiedSources) {
  ...
  return {
    ...
    batch: 'batch_7_procedure_hardening_states_v1',
...
function buildStateReport(stateRow, summary, gapRows, failureRows, verifiedSources, nextActions) {
  return [
    `# ${stateRow.stateName} California-Grade Batch 7 Report v1`,
    '',
...
function buildBatchReport(batchSummary) {
  return [
    '# Batch 7 Procedure Hardening States Report v1',
    '',
    'This pass creates state-specific California-grade repair control-plane artifacts for Alabama, South Dakota, and Louisiana using the hardened Texas v10 rules. It does not promote any state without full critical-family verification.',
    '',
    '## State status',
    '',
    ...
    '',
    '- No new reusable lesson was promoted from Batch 7; existing Texas v10 rules and the current lessons docs remain authoritative for the next batch.',
  ].join('\n');
}

export function generateBatch7ProcedureHardeningStatesV1() {
  ...
  try {
    ...
    writeJson(path.join(generatedDir, 'batch7_procedure_hardening_states_summary_v1.json'), batchSummary);
    fs.writeFileSync(path.join(docsGeneratedDir, 'batch7-procedure-hardening-states-report-v1.md'), `${buildBatchReport(batchSummary)}\n`);
    return batchSummary;
  } finally {
    db.close();
  }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const summary = generateBatch7ProcedureHardeningStatesV1();
