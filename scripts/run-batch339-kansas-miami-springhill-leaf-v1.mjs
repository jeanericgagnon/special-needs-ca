...
const OUTPUTS = {
  summary: path.join(generatedDir, 'batch339_kansas_miami_springhill_leaf_summary_v1.json'),
  report: path.join(docsGeneratedDir, 'batch339-kansas-miami-springhill-leaf-report-v1.md'),
};

const COUNT = 26;
const PRIMARY_GAP_REASON =
  'current_ksde_directory_roots_and_pdf_url_return_request_rejected_shells_and_exact_submit_replay_is_rejected_while_reviewed_local_district_leaves_cover_only_26_counties';
const FAMILY_STATUS =
  'blocked_reviewed_local_kansas_district_leaves_expand_to_26_counties_but_current_live_ksde_submit_replay_is_rejected';
const NEXT_ACTION =
...
const BATCH = 'batch339_kansas_miami_springhill_leaf_v1';

const NEW_LEAF = {
  county_id: 'miami-ks',
  county_name: 'miami',
  district_name: 'Spring Hill Schools USD 230',
  district_website: 'https://www.usd230.org/',
  source_url: 'https://www.usd230.org/page/special-education',
  final_url: 'https://www.usd230.org/page/special-education',
  verification_status: 'verified',
  source_type: 'district_owned_special_education_leaf',
  fetched_at: '2026-06-24T00:00:00.000Z',
  evidence_title: 'Special Education | SPRING HILL SCHOOLS',
  evidence_h1: null,
  evidence_snippet:
    'The official Spring Hill Schools sitemap exposes an exact district-owned `/page/special-education` leaf, and the fetched usd230.org page returned HTTP 200 with title `Special Education | SPRING HILL SCHOOLS` plus H2 `Special Education` on the district-owned host.',
};
...
function buildStateReport(summary, gapRows, failureRows, verifiedRows, nextRows) {
  return [
    ...
    '- Miami now clears from the district-owned Spring Hill Schools `Special Education` leaf.',
...
function updateAllStateReport() {
  const note =
    '- Kansas remains blocked because the live KSDE Directory Reports root, Directories page, educational-directory PDF URL, and exact district-scoped submit replay still collapse to `Request Rejected` shells, even though reviewed local district-owned or district-linked special-education leaves now cover 26 of 105 counties, including the district-owned Spring Hill Schools `Special Education` leaf for Miami County.';
...
function buildBatchReport() {
  return [
    '# Batch 339 Kansas Miami Spring Hill Leaf v1',
    '',
    ...
    '',
    '- Miami now clears from the district-owned Spring Hill Schools `Special Education` leaf.',
    '- The official Spring Hill Schools sitemap exposes the exact `/page/special-education` route on the district-owned host.',
    '- The fetched Spring Hill district leaf preserves the role-exact title `Special Education | SPRING HILL SCHOOLS` and H2 `Special Education` on the district host.',
...
export function generateBatch339KansasMiamiSpringhillLeafV1() {
  ...
  const updatedPacket = {
    ...
    current_problem_metrics: {
      ...
      authoredExactLeafCount: 26,
      reviewedDistrictOwnedLeafCount: 26,
    },
    reviewed_local_leaf_counties: Array.from(new Set([...(packet.reviewed_local_leaf_counties || []), 'miami-ks'])).sort(),
    unresolved_local_leaf_counties: (packet.unresolved_local_leaf_counties || []).filter((countyId) => countyId !== 'miami-ks'),
  };
  ...
  const batchSummary = {
    ...
    classification: 'BLOCKED',
    index_safe: false,
    promoted_county: 'miami-ks',
    promoted_counties_total: 26,
    district_root_status: 200,
    district_root_title: 'Home | SPRING HILL SCHOOLS',
    district_leaf_status: 200,
    district_leaf_title: 'Special Education | SPRING HILL SCHOOLS',
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const result = generateBatch339KansasMiamiSpringhillLeafV1();
