...
const OUTPUTS = {
  summary: path.join(generatedDir, 'batch345_kansas_franklin_ottawa_student_services_leaf_summary_v1.json'),
  report: path.join(docsGeneratedDir, 'batch345-kansas-franklin-ottawa-student-services-leaf-report-v1.md'),
};
...
const BATCH = 'batch345_kansas_franklin_ottawa_student_services_leaf_v1';

const NEW_LEAF = {
  county_id: 'franklin-ks',
  county_name: 'franklin',
  district_name: 'Ottawa USD 290',
  district_website: 'https://www.usd290.org/',
  source_url: 'https://sites.google.com/usd290.org/studentlearningservices/special-education',
  final_url: 'https://sites.google.com/usd290.org/studentlearningservices/special-education',
  verification_status: 'verified',
  source_type: 'district_linked_special_education_google_site_leaf',
  fetched_at: '2026-06-24T00:00:00.000Z',
  evidence_title: 'USD 290 Student Services - Special Education',
  evidence_h1: null,
  evidence_snippet:
    'The official Ottawa USD 290 homepage links directly to the district-branded `studentlearningservices` site on the `usd290.org` namespace, and the fetched exact `Special Education` leaf returned HTTP 200 with title `USD 290 Student Services - Special Education` while the same district-linked stack also exposes `Parent Rights` on the local student-services tree.',
};
...
function buildStateReport(summary, gapRows, failureRows, verifiedRows, nextRows) {
  return [
    ...
    '- Franklin now clears from the district-linked Ottawa USD 290 Student Services `Special Education` leaf.',
...
function updateAllStateReport() {
  const note =
    '- Kansas remains blocked because the live KSDE Directory Reports root, Directories page, educational-directory PDF URL, and exact district-scoped submit replay still collapse to `Request Rejected` shells, even though reviewed local district-owned or district-linked special-education leaves now cover 30 of 105 counties, including Ottawa USD 290’s district-linked Student Services `Special Education` leaf for Franklin County.';
...
function buildBatchReport() {
  return [
    '# Batch 345 Kansas Franklin Ottawa Student Services Leaf v1',
    '',
    ...
    '',
    '- Franklin now clears from the district-linked Ottawa USD 290 Student Services `Special Education` leaf.',
    '- The official Ottawa USD 290 homepage links directly to the district-branded `studentlearningservices` site on the `usd290.org` namespace.',
    '- The fetched exact `Special Education` page returned HTTP 200 with title `USD 290 Student Services - Special Education`, and the same local stack also exposes `Parent Rights`.',
...
export function generateBatch345KansasFranklinOttawaStudentServicesLeafV1() {
  ...
  const updatedPacket = {
    ...
    current_problem_metrics: {
      ...
      authoredExactLeafCount: 30,
      reviewedDistrictOwnedLeafCount: 30,
    },
    reviewed_local_leaf_counties: Array.from(new Set([...(packet.reviewed_local_leaf_counties || []), 'franklin-ks'])).sort(),
    unresolved_local_leaf_counties: (packet.unresolved_local_leaf_counties || []).filter((countyId) => countyId !== 'franklin-ks'),
  };
  ...
  const batchSummary = {
    ...
    classification: 'BLOCKED',
    index_safe: false,
    promoted_county: 'franklin-ks',
    promoted_counties_total: 30,
    district_root_status: 200,
    district_root_title: 'Home | Ottawa USD 290',
    district_leaf_status: 200,
    district_leaf_title: 'USD 290 Student Services - Special Education',
    district_leaf_h1: null,
  };
