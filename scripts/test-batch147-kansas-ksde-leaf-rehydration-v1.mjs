...
assert.equal(summary.primary_gap_reason, 'kansas_dd_authority_still_access_blocked_and_no_county_or_district_education_contract_preserved');
...
assert.equal(verifiedByFamily.get('district_or_county_education_routing').sample_count, 1);
...
assert.match(verifiedByFamily.get('district_or_county_education_routing').samples[0].evidence_snippet, /county-to-district join/i);
...
assert.equal(kansasQueue.completeness_pct, 83);
assert.equal(kansasQueue.primary_gap_reason, 'kansas_dd_authority_still_access_blocked_and_no_county_or_district_education_contract_preserved');

const kansasAudit = audit.states.find((row) => row.stateId === 'kansas');
assert.equal(kansasAudit.completenessPct, 83);
assert.equal(kansasAudit.strongCriticalFamilies, 10);
assert.equal(kansasAudit.weakCriticalFamilies, 2);
...
assert.ok(report.includes('early intervention and statewide special education remain verified'));
