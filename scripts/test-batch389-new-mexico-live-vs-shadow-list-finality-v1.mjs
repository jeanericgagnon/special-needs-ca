...
assert.match(educationGap.status_reason, /only matches 58 of 142 normalized superintendent-district names/i);
assert.match(educationGap.status_reason, /leaving 84 official district names outside the REC grouping/i);
...
assert.match(educationFailure.evidence, /only 58 matched district names out of 142 normalized superintendent-district names/i);
assert.match(educationFailure.evidence, /leaving 84 official district names outside the REC grouping/i);
...
assert.equal(batchSummary.rec_normalized_match_count, 58);
assert.equal(batchSummary.superintendent_normalized_district_count, 142);
assert.equal(batchSummary.rec_normalized_missing_count, 84);
...
assert.equal(batchSummary.blocker_changed, true);
assert.match(batchSummary.conclusion, /58 of 142 normalized superintendent districts/i);
