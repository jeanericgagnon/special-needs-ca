...
import { generateBatch87VermontStatewideFamilyTruthRefreshV1 } from './run-batch87-vermont-statewide-family-truth-refresh-v1.mjs';

const repoRoot = path.resolve(path.dirname(new URL(import.meta.url).pathname), '..');

const { summary, batchSummary } = generateBatch87VermontStatewideFamilyTruthRefreshV1();
...
assert.deepEqual(batchSummary.resolved_families, ['legal_aid']);
...
assert.match(legalAid.samples[0].evidence_snippet, /Get Free Legal Help/i);
...
assert.match(report, /reviewed first-party Vermont Legal Aid evidence preserves statewide free civil legal-help routing/i);
assert.match(report, /still lacks a reviewed PTI-designation artifact chain/i);
