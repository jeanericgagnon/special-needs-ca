...
function updateAllStateReport(report) {
  ...
  const previousLine = '- Idaho remains blocked after a final live sitemap pass: Camas only exposes contact routing plus a board-roster Google Doc and a 404 sitemap, Clark only exposes Contact Us, Title IX, general-education notice attachments, image-only Child Find flyers, and a 404 sitemap, and the live Idaho DHW sitemap confirms office inventory but still no county-to-office contract.';
  const newLine = '- Idaho remains blocked after a final live sitemap pass: Camas now exposes contact routing, a board-roster Google Doc, same-host Federal Programs and Advanced Opportunities pages, and a 404 sitemap, but those rendered leaves are still the wrong role for local special-education routing; Clark now has dead short aliases but live replacement Contact Us, Title IX, and general-education notice leaves that are still the wrong role; and the live Idaho DHW sitemap confirms office inventory but still no county-to-office contract.';
  const genericPattern = /- Idaho remains blocked after a final live sitemap pass:[^\n]*/;
  if (genericPattern.test(report)) return report.replace(genericPattern, newLine);
  if (report.includes(previousLine)) return report.replace(previousLine, newLine);
  if (report.includes(oldLine)) return report.replace(oldLine, newLine);
  return report;
}
