...
import { generateBatch5ProcedureHardeningStatesV1 } from './run-batch5-procedure-hardening-states-v1.mjs';
...
generateBatch5ProcedureHardeningStatesV1();

const batchSummary = readJson('data/generated/batch5_procedure_hardening_states_summary_v1.json');
...
const batchReport = fs.readFileSync(path.join(docsGeneratedDir, 'batch5-procedure-hardening-states-report-v1.md'), 'utf8');

assert.deepEqual(batchSummary.states.map((row) => row.state), ['iowa', 'mississippi', 'minnesota'], 'Batch 5 must cover Iowa, Mississippi, and Minnesota in order');
assert.equal(batchSummary.generated_all_state_v3, false, 'Batch 5 must not generate all-state v3 unless a state actually becomes complete');
assert.equal(batchSummary.complete_states.length, 0, 'No Batch 5 state should be promoted complete from current evidence');
assert.equal(batchSummary.texas_preserved_complete, true, 'Texas must remain COMPLETE/index_safe while Batch 5 runs');
...
for (const stateId of ['iowa', 'mississippi', 'minnesota']) {
...
assert.ok(batchReport.includes('generated_all_state_v3: false'), 'Batch 5 report must explicitly say all-state v3 was not generated');
assert.ok(batchReport.includes('texas_preserved_complete: true'), 'Batch 5 report must explicitly preserve Texas complete truth');
