...
function buildStateReport(summary, gapRows, failureRows, verifiedRows, nextRows) {
  return [
    ...
    '- Education is still an exact reviewed-leaf expansion lane, but it is stronger than before: six counties now have reviewed district-owned special-education or special-services leaves.',
    '- Boundary, Butte, and Bonneville were promoted from official district sitemap exact leaves; Blaine remains signal-only until a role-exact local leaf is verified.',
...
export function generateBatch212IdahoBonnevilleReviewedDistrictLeafV1() {
  ...
  const updatedPacket = {
    ...
    current_problem_metrics: {
      ...
      authoredExactLeafCount: 6,
      reviewedExactLeafCount: 6,
    },
    packet_complete_when: 'At least one reviewed district-owned education-routing leaf is attached for every Idaho county without relying on statewide SDE fallbacks or county-name inference alone. The current packet now has reviewed exact leaves for Cassia, Payette, Bannock, Boundary, Butte, and Bonneville as seed examples.',
  ...
  const batchSummary = {
    ...
    reviewed_exact_district_leaves: 6,
    promoted_from_sitemap_exact_leaves: 3,
  ...
  const batchReport = [
    ...
    '',
    '- Idaho remains blocked and not index-safe.',
    '- Bonneville Joint District #93 now joins Cassia, Payette, Bannock, Boundary, and Butte as a reviewed district-owned education-routing leaf.',
    '- Boundary, Butte, and Bonneville were promoted from official district sitemap exact leaves.',
