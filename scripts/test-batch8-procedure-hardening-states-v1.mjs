...
import { generateBatch8ProcedureHardeningStatesV1 } from './run-batch8-procedure-hardening-states-v1.mjs';
...
generateBatch8ProcedureHardeningStatesV1();

const batchSummary = readJson('data/generated/batch8_procedure_hardening_states_summary_v1.json');
...
const batchReport = fs.readFileSync(path.join(docsGeneratedDir, 'batch8-procedure-hardening-states-report-v1.md'), 'utf8');

assert.deepEqual(batchSummary.states.map((row) => row.state), ['tennessee', 'virginia', 'indiana'], 'Batch 8 must cover Tennessee, Virginia, and Indiana in order');
assert.equal(batchSummary.generated_all_state_v3, false, 'Batch 8 must not generate all-state v3 unless a state actually becomes complete');
assert.equal(batchSummary.complete_states.length, 0, 'No Batch 8 state should be promoted complete from current evidence');
assert.equal(batchSummary.texas_preserved_complete, true, 'Texas must remain COMPLETE/index_safe while Batch 8 runs');
...
for (const stateId of ['tennessee', 'virginia', 'indiana']) {
...
assert.ok(batchReport.includes('generated_all_state_v3: false'), 'Batch 8 report must explicitly say all-state v3 was not generated');
assert.ok(batchReport.includes('texas_preserved_complete: true'), 'Batch 8 report must explicitly preserve Texas complete truth');
