...
const LIVE_PROBES = {
  medicaid: {
    url: 'https://chfs.ky.gov/dms',
    finalUrl: 'https://chfs.ky.gov/dms',
    status: 404,
    contentType: 'text/html',
    evidence: 'Reviewed 2026-06-21 targeted live probe returned HTTP 404 for the exact Kentucky Medicaid packet URL.',
  },
  specialEducation: {
    url: 'https://education.kentucky.gov/',
    finalUrl: '',
    status: 0,
    contentType: '',
    evidence: 'Reviewed 2026-06-21 targeted live probe failed DNS resolution for education.kentucky.gov, so the packet special-education root is not a live reviewed authority source.',
  },
  ddAuthority: {
  ...
  waiver: {
    url: 'https://dbhdid.ky.gov/hcbs/eligibility',
    finalUrl: 'https://dbhdid.ky.gov/hcbs/eligibility',
    status: 200,
    contentType: 'text/html',
    evidence: 'Reviewed 2026-06-21 Playwright render returned only "Loading to Department for Behavioral Health page." with no waiver eligibility, application, or action text.',
  },
};
  ...
  return [
    {
      ...
      family_status: 'missing_reviewed_role_pure_medicaid_source',
      evidence_strength: 'missing',
      sample_count: 0,
      query_basis: 'The old Kentucky Medicaid packet sample chain was rechecked against the exact CHFS / DMS source and the exact URL now returns HTTP 404.',
      blocker_code: 'live_medicaid_root_404_and_no_reviewed_replacement',
      blocker_evidence: LIVE_PROBES.medicaid.evidence,
      samples: [],
    },
    {
      state: 'kentucky',
      ...
      family_status: 'blocked_js_shell_waiver_target_unverified',
      evidence_strength: 'weak',
      sample_count: 0,
      query_basis: 'The exact reviewed DBHDID HCBS eligibility target exists, but the reviewed Playwright render produced only a loading shell instead of waiver content.',
      blocker_code: 'reviewed_exact_target_only_returns_js_loading_shell',
      blocker_evidence: LIVE_PROBES.waiver.evidence,
      samples: [],
    },
    {
      state: 'kentucky',
    ...
    },
    {
      ...
      evidence_strength: 'missing',
      sample_count: 0,
      query_basis: 'The old Early Start packet URL is stale and the remaining DBHDID program root is DD-related, not Kentucky Part C / First Steps routing.',
      blocker_code: 'legacy_early_intervention_source_dead_and_no_reviewed_replacement',
      blocker_evidence: 'Kentucky still lacks a reviewed role-aligned Part C / First Steps source on disk; the old dhhs.kentucky.gov/earlystart family is stale and no reviewed replacement was preserved in the packet.',
      samples: [],
    },
    {
      state: 'kentucky',
      ...
      family_status: 'missing_reviewed_state_special_education_source',
      evidence_strength: 'missing',
      sample_count: 0,
      query_basis: 'The packet special-education authority row still points to education.kentucky.gov, which is not a live reviewed authority source.',
      blocker_code: 'state_special_education_root_not_live_reviewed',
      blocker_evidence: LIVE_PROBES.specialEducation.evidence,
      samples: [],
    },
    {
      state: 'kentucky',
...
function buildReport(summary, gapRows, failureRows, verifiedRows, nextRows) {
  return [
    ...
    `- Kentucky Medicaid state coverage is downgraded because the exact CHFS/DMS packet source now fails live review with HTTP ${LIVE_PROBES.medicaid.status}, and the prior packet incorrectly mixed ABLE and early-intervention rows into the Medicaid family.`,
...
export function generateBatch44KentuckyStatewideFamilyTruthRefreshV1() {
  ...
  const reasonByFamily = {
    medicaid_state_health_coverage: 'The old CHFS / DMS packet root is live-broken and the prior packet mixed unrelated ABLE and early-intervention rows into the Medicaid family.',
    medicaid_waiver_hcbs_disability_services: 'The exact reviewed DBHDID HCBS target still renders only a loading shell and does not preserve waiver action text or eligibility proof.',
    ...
    early_intervention_part_c: 'Kentucky still lacks a reviewed role-aligned Part C / First Steps source on disk after the stale Early Start packet family was rejected.',
    special_education_idea_part_b: 'The packet special-education authority row points to education.kentucky.gov, which is not a live reviewed KDE authority source.',
  ...
  const updatedFailureRows = [
    {
      ...
      family: 'medicaid_state_health_coverage',
      severity: 'critical',
      failure_code: 'live_medicaid_root_404_and_no_reviewed_replacement',
      evidence: `${LIVE_PROBES.medicaid.evidence} The packet also mixed unrelated ABLE and early-intervention rows into the Medicaid family.`,
      next_action: 'author_or_review_current_kentucky_medicaid_leaf',
    },
    {
      state: 'kentucky',
      state_code: 'KY',
      ...
      evidence: LIVE_PROBES.waiver.evidence,
      next_action: 'browser_assisted_or_api_backed_dbhdid_waiver_capture',
    },
    {
      state: 'kentucky',
      state_code: 'KY',
    ...
    },
    {
      ...
      evidence: 'Kentucky still has no reviewed role-aligned Part C / First Steps source on disk after the old Early Start packet family went stale.',
    ...
    },
    {
      ...
      family: 'special_education_idea_part_b',
      severity: 'critical',
      failure_code: 'state_special_education_root_not_live_reviewed',
      evidence: LIVE_PROBES.specialEducation.evidence,
      next_action: 'author_or_review_live_kde_special_education_leaf',
    },
    {
      state: 'kentucky',
      state_code: 'KY',
