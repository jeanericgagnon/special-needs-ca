...
const OUTPUTS = {
  ...
  nextActionV7: path.join(generatedDir, 'tx_next_action_queue_v7.jsonl'),
};

const USER_AGENT = 'Ablefull Texas residual district repair/1.0 (+https://ablefull.com)';
const FETCH_TIMEOUT_MS = 8000;
const COUNTY_CONCURRENCY = 6;
const MAX_FETCHES_PER_DISTRICT = 20;
const MAX_SEARCH_QUERIES_PER_COUNTY = 5;
...
const TERM_PATTERNS = [
  ...
  { label: 'special populations', regex: /\bspecial populations\b/i, strength: 'strong' },
...
function extractRobotsSitemaps(fetchResult) {
  if (!fetchResult.ok || !/text\/plain|plain/i.test(fetchResult.contentType || '')) return [];
...
function classifyEvidence(result) {
  const headerBlob = sanitizeText(`${result.finalUrl} ${result.title} ${result.headings.join(' ')}`);
  const blob = sanitizeText(`${headerBlob} ${result.snippet}`);
  const terms = evidenceTermsFound(blob);
  if (hasBadMarker(blob)) {
    return { status: 'reject', reason: 'source_fetch_failed', terms };
  }
  if (hasForbiddenEducationSignals(headerBlob)) {
...
async function searchFallbackCandidates(candidate, searchCache, usedQueries, searchLog) {
  ...
  const queries = [
    ...
    `"${candidate.district_name}" special programs`,
    `"${candidate.district_name}" special populations`,
    `"${candidate.district_name}" special services`,
  ];
...
async function repairHomepageCandidates(candidate, searchCache, repairLog) {
  const candidates = [];
  const queries = [
    `"${candidate.district_name}" official site`,
    `"${candidate.district_name}" ISD`,
  ];
  for (const query of queries) {
    const searchUrl = `https://duckduckgo.com/html/?q=${encodeURIComponent(query)}`;
    const page = await fetchWithCache(searchUrl, searchCache);
    repairLog.push({ query, url: searchUrl, status: page.status, fetched_at: page.fetchedAt });
    if (!page.ok || !/html/i.test(page.contentType)) continue;
    const $ = cheerio.load(page.text);
    $('a[href]').each((_, el) => {
      if (candidates.length >= 8) return;
      ...
      const absolute = extractSearchResultUrl(href);
      if (!absolute) return;
      if (/duckduckgo|facebook|instagram|x\.com|twitter|greatschools|schooldigger|niche|askted|tea\.texas\.gov/i.test(absolute)) return;
      candidates.push({ url: absolute, text, query });
    });
  }
  return candidates;
}
...
async function inspectDistrictCandidate(candidate, fetchCache, searchCache, usedQueries, searchLog) {
  ...
  if (!candidate.district_homepage) {
    return {
      ...
      failures,
      search_queries: [],
      homepage_repair_attempts: homepageRepairAttempts,
    };
  }

  let workingCandidate = { ...candidate };
  let homepage = await fetchWithCache(workingCandidate.district_homepage, fetchCache);
  tried.push(homepage.finalUrl || candidate.district_homepage);
  if (!homepage.ok) {
    failures.push({ url: candidate.district_homepage, reason: 'district_homepage_broken' });
    const repairedHomepages = await repairHomepageCandidates(candidate, searchCache, homepageRepairAttempts);
    for (const repair of repairedHomepages) {
      const repairPage = await fetchWithCache(repair.url, fetchCache);
      tried.push(repairPage.finalUrl || repair.url);
      if (!repairPage.ok) continue;
      const repairHost = normalizeHost(new URL(repairPage.finalUrl).hostname);
      const repairBlob = sanitizeText(`${repairPage.title} ${repairPage.headings.join(' ')} ${repairPage.snippet}`);
      if (!/independent school district|isd|consolidated|common school district|cisd/i.test(repairBlob)) continue;
      if (!repairBlob.toLowerCase().includes(candidate.district_name.toLowerCase().replace(/\s+/g, ' ').trim().split(' ').slice(0, 2).join(' '))) continue;
      workingCandidate = { ...candidate, district_homepage: urlOrigin(repairPage.finalUrl), repaired_homepage_from: candidate.district_homepage };
      homepage = repairPage;
      homepageRepairAttempts.push({ attempted_url: repair.url, resolved_homepage: workingCandidate.district_homepage, status: repairPage.status, result: 'repaired' });
      break;
    }
    if (!homepage.ok) {
      return {
  ...
  const homepageHost = normalizeHost(new URL(homepage.finalUrl || workingCandidate.district_homepage).hostname);
  const fetchQueue = [];
  const seen = new Set();
  ...
  pushCandidate(pathJoin(workingCandidate.district_homepage, '/sitemap.xml'), 'sitemap');
  pushCandidate(pathJoin(workingCandidate.district_homepage, '/robots.txt'), 'robots');
  pushCandidate(pathJoin(workingCandidate.district_homepage, '/search'), 'site_search');
  pushCandidate(pathJoin(workingCandidate.district_homepage, '/search-results'), 'site_search');
  for (const pathname of DISTRICT_PATH_CANDIDATES) pushCandidate(pathJoin(workingCandidate.district_homepage, pathname), 'path_guess');
  ...
  if (homepageTerms.length > 0 && !hasBadMarker(homepageBlob) && !hasForbiddenEducationSignals(homepageBlob)) {
    homepageOnlyResult = {
      source_url: workingCandidate.district_homepage,
  ...
  while (fetchQueue.length && fetches < MAX_FETCHES_PER_DISTRICT) {
    ...
    if (job.method === 'robots') {
      for (const url of extractRobotsSitemaps(result)) pushCandidate(url, 'sitemap');
      continue;
    }
...
function recommendedNextAction(category) {
  if (category === 'district_homepage_broken') return 'repair official district homepage/domain and rerun district-grade verification';
  if (category === 'weak_student_services_only') return 'find stronger same-domain special education, child find, 504, or dyslexia evidence';
  if (category === 'search_fallback_exhausted') return 'author exact district-owned target URLs manually from official district navigation or document indexes';
  return 'manual district-grade review required';
}

function buildFailureLedgerEntry(county, candidatesTried, urlsTried, reason, category, searchQueries = [], homepageRepairAttempts = []) {
  return {
    county_slug: county.county_slug,
    district_candidates_tried: candidatesTried,
    homepage_domain_repair_attempts: homepageRepairAttempts,
    urls_tried: urlsTried,
    search_queries: searchQueries,
    reason,
    category,
    final_reason: reason,
    recommended_manual_next_action: recommendedNextAction(category),
  };
}

async function buildV7Education(partialCounties, directSourcesV6, askTedByCounty, fetchCache, searchCache) {
  const results = [];
  const failureLedger = [];
  let completed = 0;
  ...
  async function processCounty(county) {
    ...
    return {
      ...
      failure: buildFailureLedgerEntry(
        ...
        countySearchLog,
        fallback.inspection.homepage_repair_attempts || [],
      ),
    };
  }
  ...
  async function worker() {
    while (queue.length) {
      ...
      if (completed % 10 === 0 || completed === partialCounties.length) {
        console.log(`tx-v7 progress ${completed}/${partialCounties.length}`);
      }
    }
  }
  ...
  const passthrough = directSourcesV6
...
function buildCountyBaselineV7(countiesV6, directSourcesByCounty) {
  return countiesV6.map((row) => {
...
function buildSummaryV7(summaryV6, countiesV7, failuresV7) {
  const partialRows = countiesV7.filter((row) => row.verification_status === 'partial');
  ...
  return {
    ...
    v6: summaryV6.v6,
    v7: {
      pass_counties: countiesV7.filter((row) => row.verification_status === 'pass').length,
      partial_counties: partialRows.length,
      blocked_counties: countiesV7.filter((row) => row.verification_status === 'blocked').length,
    },
    attempted_partial_counties: summaryV6.v6.partial_counties,
    repaired_to_pass: countiesV7.filter((row) => row.verification_status === 'pass').length - summaryV6.v6.pass_counties,
    ...
    top_failure_categories: countBy(failuresV7, (row) => row.category),
    counties_below_california_grade: partialRows.map((row) => row.county_slug),
    index_safe: countiesV7.every((row) => row.verification_status === 'pass'),
  };
}

function buildNextActionQueueV7(failuresV7) {
  return Object.entries(countBy(failuresV7, (row) => row.category))
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([category, count]) => ({
...
function buildReport(summaryV7, filesChanged) {
  return [
    ...
    '## Result history',
    '',
    `- v6 PASS/PARTIAL/BLOCKED: ${summaryV7.v6.pass_counties}/${summaryV7.v6.partial_counties}/${summaryV7.v6.blocked_counties}`,
    `- v7 PASS/PARTIAL/BLOCKED: ${summaryV7.v7.pass_counties}/${summaryV7.v7.partial_counties}/${summaryV7.v7.blocked_counties}`,
    '',
    '## Residual repair outcome',
    '',
    `- Attempted counties: ${summaryV7.attempted_partial_counties}`,
    `- Repaired counties: ${summaryV7.repaired_to_pass}`,
    `- Still partial counties: ${summaryV7.still_partial}`,
    '',
    '## Counties still partial with exact reason',
    '',
    ...summaryV7.partial_reasons.map((row) => `- ${row.county_slug}: ${row.reason} (${row.category})`),
    '',
    '## Top failure categories',
    '',
    ...Object.entries(summaryV7.top_failure_categories).map(([category, count]) => `- ${category}: ${count}`),
    '',
    '## Counties below California-grade',
    '',
    ...summaryV7.counties_below_california_grade.map((slug) => `- ${slug}`),
    '',
    '## Texas index-safe',
    '',
    `- Texas is index-safe: ${summaryV7.index_safe ? 'yes' : 'no'}`,
    '',
    '## Tests run',
    '',
    ...
    '## Next action',
    '',
    summaryV7.index_safe ? '- none' : '- Keep the remaining partial counties gated and repair the residual district-grade evidence gaps county-by-county.',
  ].join('\n');
}

async function main() {
  const countiesV6 = readJsonl(INPUTS.countyV6);
  const directSourcesV6 = readJsonl(INPUTS.directSourcesV6);
  const summaryV6 = readJson(INPUTS.summaryV6);
  const csvRows = parseCsv(fs.readFileSync(INPUTS.asktedCsvV3, 'utf8'));
  const partialCounties = countiesV6.filter((row) => row.verification_status === 'partial');
  const askTedByCounty = districtCandidatesByCounty(csvRows);
  const fetchCache = new Map();
  const searchCache = new Map();

  const { directSources, failureLedger } = await buildV7Education(partialCounties, directSourcesV6, askTedByCounty, fetchCache, searchCache);
  const directSourcesByCounty = new Map(directSources.map((row) => [row.county_slug, row]));
  const countiesV7 = buildCountyBaselineV7(countiesV6, directSourcesByCounty);
  const summaryV7 = buildSummaryV7(summaryV6, countiesV7, failureLedger);
  const nextActions = buildNextActionQueueV7(failureLedger);
  ...
  const report = buildReport(summaryV7, filesChanged);

  writeJsonl(OUTPUTS.directSourcesV7, directSources);
  writeJsonl(OUTPUTS.countyV7, countiesV7);
  writeJsonl(OUTPUTS.failureV7, failureLedger);
  writeJsonl(OUTPUTS.nextActionV7, nextActions);
  writeJson(OUTPUTS.summaryV7, summaryV7);
  fs.mkdirSync(path.dirname(OUTPUTS.reportV7), { recursive: true });
  fs.writeFileSync(OUTPUTS.reportV7, `${report}\n`);

  console.log(JSON.stringify({
    ok: true,
    v6: summaryV7.v6,
    v7: summaryV7.v7,
    repaired: summaryV7.repaired_to_pass,
    still_partial: summaryV7.still_partial,
    index_safe: summaryV7.index_safe,
  }, null, 2));
}
