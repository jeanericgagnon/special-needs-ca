...
const BATCH_STATES = ['california', 'pennsylvania', 'florida'];
const STATE_CODE = { california: 'CA', pennsylvania: 'PA', florida: 'FL' };
const STATE_LABEL = { california: 'California', pennsylvania: 'Pennsylvania', florida: 'Florida' };
...
function buildStateSummary(stateRow, stateFailures, priorityRow, verifiedSources) {
  ...
  return {
    ...
    batch: 'batch_1_fast_finish_states_v1',
...
function buildStateReport(stateRow, summary, gapRows, failureRows, verifiedSources, nextActions) {
  return [
    `# ${stateRow.stateName} California-Grade Batch 1 Report v1`,
    '',
...
function buildBatchReport(batchSummary) {
  return [
    '# Batch 1 Fast Finish States Report v1',
    '',
    'This pass creates state-specific California-grade repair control-plane artifacts for California, Pennsylvania, and Florida using the hardened Texas v10 rules. It does not promote any state without full critical-family verification.',
    '',
    '## State status',
    '',
...
export function generateBatch1FastFinishStatesV1() {
  ...
  try {
    ...
    writeJson(path.join(generatedDir, 'batch1_fast_finish_states_summary_v1.json'), batchSummary);
    fs.writeFileSync(path.join(docsGeneratedDir, 'batch1-fast-finish-states-report-v1.md'), `${buildBatchReport(batchSummary)}\n`);
    return batchSummary;
  } finally {
    db.close();
  }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const summary = generateBatch1FastFinishStatesV1();
