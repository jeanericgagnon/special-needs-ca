...
assert.equal(summary.completeness_pct, 83, 'Colorado completeness must reflect the legal-aid repair while two local-proof blockers remain.');
assert.equal(summary.strong_critical_families, 10, 'Colorado should retain ten strong critical families after the truth refresh.');
assert.equal(summary.weak_critical_families, 2, 'Colorado should retain two weak critical families after the truth refresh.');
assert.equal(summary.missing_critical_families, 0, 'Colorado should no longer retain a missing critical family after the legal-aid repair.');
assert.equal(summary.primary_gap_reason, 'district_grade_education_and_county_local_office_proof_still_unverified');
...
assert.ok(!failureRows.some((row) => row.family === 'legal_aid'), 'Colorado legal aid must drop out of the failure ledger once Colorado Legal Services is reviewed.');
...
assert.equal(verifiedByFamily.get('legal_aid').sample_count, 1, 'Colorado legal aid must carry one reviewed first-party sample.');
assert.equal(verifiedByFamily.get('legal_aid').samples[0].final_url, 'https://www.coloradolegalservices.org/');
...
assert.ok(report.includes('Colorado Legal Services is explicit enough for legal aid'), 'Colorado report must explain the legal-aid repair precisely.');
