...
*   **Lesson:** If a reviewed statewide services page only confirms Alaska Connect, a virtual contact center, or a generic `Office locations` link, that is still not a county-equivalent routing contract unless the linked surface itself assigns boroughs or census areas. Alaska's reviewed DPA Services page proved the services lane was live, but it only looped back to the same region-only office page and statewide contacts.
*   **Lesson:** If a live official office page groups locations into custom regions that cross or mismatch county-equivalent boundaries, treat those buckets as negative evidence rather than partial county coverage. Alaska's DPA page grouped Fairbanks with Nome, Juneau with Ketchikan and Sitka, and Bethel with Kodiak, which proved the office cities could not be projected into borough or census-area assignments.
*   **Lesson:** If a state publishes live county or county-and-town program workbooks, inspect them before assuming they help the local-office lane. Maine's official OFI workbooks were real public XLSX files, but they only preserved beneficiary counts by county and town and still added no office identifiers, office names, or service-area bindings.
*   **Lesson:** If a residual district root now exposes a live sitemap, read the sitemap before guessing more slugs, then treat repeated generic-only inventories as blocker evidence. Idaho's Clark, Fremont, and Shoshone sitemaps all became public, but they still materialized only contact, Title IX, staff, documents, or generic district pages and no special-education-routing leaves.
*   **Lesson:** If a district host also exposes a documents or forms library, check that lane once before holding the blocker final, then treat a generic-only library as negative evidence rather than a discovery promise. Idaho Fremont's live `/documents` and `/page/district-forms` surfaces were public and stable, but they still contained no special-education, child-find, 504, or procedural-safeguards routing.
*   **Lesson:** If an official SharePoint directory host is public, inspect the backing list pages and REST list GUIDs before trusting workbook exports as the whole lane. New Mexico's PED school-directory host exposed a live `2017 NM Schools` list with `ItemCount: 935` plus public `items` and `RenderListDataAsStream` responses, which strengthened the blocker by proving the live schema still had no county field instead of leaving the lane as workbook-only.
*   **Lesson:** If a public REC or regional page groups districts under numbered cooperatives but never labels counties or service areas, treat it as supportive structure rather than a county-grade contract. New Mexico's live `RECHome.aspx` page proved REC groupings on the official PED host, but it still did not map counties to REC service areas.
*   **Lesson:** If a public district or office host is live, spend one bounded robots/sitemap or raw-HTML pass before guessing more leaves. Arizona's last three district domains exhausted into generic student pages and handbooks, and AHCCCS's public ALTCS lane proved seven office cards plus a partial county map but still no county-to-office contract, so neither family could be upgraded without a real local assignment surface.
*   **Lesson:** If an older official PDF path now returns a normal HTML shell instead of PDF bytes, treat that as stale evidence and prefer the current download lane before concluding the file family is missing. Arizona's old `WhereToApply.pdf` path and older University Family Care PDF paths now replayed to AHCCCS HTML, while the current `/Resources/Downloads/UniversityFamilyCare/` files stayed live and proved the bundle was support-letter collateral rather than a county contract.
...
### Challenged DOE Roots Can Still Have Accessible Official Inventory Apps
*   **Lesson:** If the main state DOE host is challenge-blocked, inspect sibling official data or report-card apps before declaring the district-root lane empty. Arizona's `azreportcards.azed.gov` host stayed public and its `/api/Entity/GetEntityList` endpoint exposed statewide district inventory even while the main AZED education pages, robots.txt, and sitemap stayed challenged.
...
*   **Lesson:** If a remaining district root only hits `504` in raw HTML or SVG/path numbers and no same-host education leaf materializes from the root, robots, or sitemap surfaces, treat that as numeric noise rather than a recovery signal. Idaho Camas and Shoshone both still emitted `504` tokens on live roots, but the bounded follow-up proved those were not real 504 or special-education routing leaves.
...
*   **Lesson:** Once a reviewed locality-capture packet proves county-grade coverage for most counties, rewrite the state blocker to the exact unresolved remainder instead of preserving a broader “no county contract” summary. Massachusetts stayed BLOCKED, but the truthful blocker is now Suffolk-only after the DDS locality capture cleared 13 of 14 counties.
...
*   **Lesson:** A district-owned homepage link to a district-branded Google Sites subtree can satisfy county-grade education routing when the linked stack carries exact `Special Education` and `Parent Rights` tabs on the district namespace. Kansas Franklin cleared once Ottawa USD 290 linked directly to `sites.google.com/usd290.org/studentlearningservices/`, whose fetched local leaves preserved the titles `USD 290 Student Services - Special Education` and `USD 290 Student Services - Parent Rights`.
...
*   **Lesson:** If an exact official page is readable in an external reviewed lane but the same URL challenge-blocks in repo-side Playwright, treat the readable content as review evidence only, not as a recoverable automation lane. Alaska’s DPA offices page stayed useful for narrowing the blocker, but repo-side Playwright still hit the Cloudflare shell and exposed no reusable county-equivalent contract.
