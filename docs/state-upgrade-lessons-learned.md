...
*   **Lesson:** If a state DOE host was previously downgraded on a transient challenge shell, re-probe the exact leaf before preserving that downgrade. Kansas KSDE later served full `Special Education` and `Early Childhood Special Education` HTML again on the same URLs, so the right move was to rehydrate the statewide families and leave only the separate district-routing contract blocked.
*   **Lesson:** If a first-party homepage is too generic to prove PTI designation, check the organization’s About or history page before leaving the family blocked. Kentucky KY-SPIN’s homepage only proved statewide parent support, but the About page explicitly preserved the original PTI grant and statewide center language needed for California-grade PTI verification.
*   **Lesson:** A statewide legal-help portal can satisfy the legal-aid family when it preserves parish search plus named legal-aid organization routing on the public page. LouisianaLawHelp cleared legal aid because the reviewed `Find Legal Help` page exposed both parish-based lookup and legal-aid organization listings without needing a single-region office to stand in for the whole state.
