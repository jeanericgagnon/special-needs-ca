...
*   **Residual-County Rule:** Once only a small set of counties remain below grade, switch from broad district discovery to a county repair lane that exhausts all district candidates in that county, records exact failure classes, and then hands `search_fallback_exhausted` counties to a manual exact-target authoring pass rather than repeating the same automated crawl patterns.
*   **Sitemap-First Rule:** In that manual exact-target lane, fetch the district sitemap before reopening search fallback. District sitemaps often reveal the exact `special-education`, `special-programs`, `dyslexia`, `504`, or district-document endpoints needed to close the county truthfully with lower token and fetch cost than another search loop.
*   **Evidence-Header Rule:** Governance-page rejection must be based on the district page’s URL, title, and headings before considering navigation-heavy body text. Otherwise valid district special-education pages can be falsely rejected because the site chrome contains words like `board`, `athletics`, or `calendar`.
*   **District-Owned Google Site Rule:** A Google Site can satisfy district-grade education proof only when the fetched final URL embeds the district-controlled domain and the fetched text explicitly proves district special-education ownership or routing. URL shape alone never counts.
*   **District-Document OCR Rule:** District-owned PDFs and image-heavy documents can satisfy the education gate only after OCR or deterministic manual-text extraction preserves the evidence snippet, contact/referral text, and document hash. Binary presence alone is not a PASS.
