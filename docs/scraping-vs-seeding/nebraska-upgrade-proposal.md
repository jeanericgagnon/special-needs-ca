# Scraping Upgrade Proposal: Nebraska (NE)

This upgrade proposal maps out the transition from seeded/programmatic fallback records to live, scraper-derived official data for Nebraska.

## 1. Current Placeholders and Fallbacks

We currently rely on **182** programmatic fallback placeholders across the state. Replacing these with verified listings is the primary goal of the scraping system.

| Category / Table | Current Fallback Count | Target Source URL | Expected Replacement Candidates | Expected Confidence |
| :--- | :--- | :--- | :--- | :--- |
| **Medicaid Offices** (`county_offices`) | 91 | Official state locator/directory | Scraped county offices | `0.90` (Official Domain) |
| **School Districts** (`school_districts`) | 91 | State DOE directory | Scraped special education contacts | `0.95` (State DOE) |
| **Nonprofits** (`nonprofit_organizations`) | 0 | N/A | Completed/Source-listed | N/A |

## 2. Upgrade Action Plan & Impact

- **Total Proposed Replacements:** 182 records
...
- Replaces programmatic office: `nebraska-medicaid-office-adams-county`
- Replaces programmatic district: `nebraska-school-district-adams-county-fallback`
