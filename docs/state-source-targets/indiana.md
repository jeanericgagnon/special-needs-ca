# State Source Targets: Indiana (IN)

This document outlines the authoritative crawler target domains, specific agencies, and extraction methods required to replace programmatic placeholders in Indiana with real, source-listed records.

> [!NOTE]
> **Category Scaffold Complete:** This state is currently mapped as a category-level scaffold (14 targets). Source-level expansion will follow in Wave 2.

## 1. Domain Crawler Targets (Wave 2)

| Target Name | Category / Subcategory | Source Domain | Crawl Method | Target Table |
| :--- | :--- | :--- | :--- | :--- |
| **Indiana County Metadata** | A. State identity and geography / County List | [indiana.gov](https://www.indiana.gov) | `static_fetch` | `counties` |
| **Indiana Medicaid Portal** | B. Medicaid / benefits / HHS / Medicaid Landing Page | [dhhs.indiana.gov](https://dhhs.indiana.gov) | `playwright` | `county_offices` |
| **Indiana Developmental Services Directory** | C. Developmental disability / DD / IDD services / State DD Agency Page | [dhhs.indiana.gov](https://dhhs.indiana.gov/dd) | `playwright` | `state_resource_agencies` |
| **Indiana HCBS Waivers Page** | D. HCBS waivers / Waiver Page | [dhhs.indiana.gov](https://dhhs.indiana.gov/dd/waivers) | `static_fetch` | `programs` |
| **Indiana Early Intervention / Part C** | E. Early intervention / Early Intervention Landing Page | [dhhs.indiana.gov](https://dhhs.indiana.gov/earlyintervention) | `static_fetch` | `programs` |
| **Indiana Department of Education Special Ed** | F. Special education / IEP / SEA Special Ed Page | [education.indiana.gov](https://education.indiana.gov) | `static_fetch` | `programs` |
| **Indiana Regional Special Education Support** | G. Regional education structures / Regional Agency Directory | [education.indiana.gov](https://education.indiana.gov/regional) | `playwright` | `regional_education_agencies` |
| **Indiana Parent Training Center** | H. Parent training / disability rights / legal aid / PTI Center | [parentcenterhub.org](https://www.parentcenterhub.org) | `static_fetch` | `nonprofit_organizations` |
| **The Arc of Indiana** | I. Condition-specific nonprofits / The Arc State Chapter | [thearcindiana.org](https://www.thearcindiana.org) | `static_fetch` | `nonprofit_organizations` |
| **Special Education Attorneys Directory** | J. Provider and advocate directories / Attorney Directory | [copaa.org](https://www.copaa.org) | `playwright` | `iep_advocates` |
| **Indiana Medicaid & Special Education Forms** | K. Forms and guides / Forms Library | [dhhs.indiana.gov](https://dhhs.indiana.gov/forms) | `pdf_extract` | `forms` |
| **Indiana Vocational Rehabilitation Services** | L. Transition / adult services / Vocational Rehabilitation | [dhhs.indiana.gov](https://dhhs.indiana.gov/rehab) | `static_fetch` | `programs` |
| **Indiana Children's Hospital Clinics** | M. Hospitals / university clinics / Hospitals | [childrenshospital.org](https://www.childrenshospital.org) | `manual_review` | `resource_providers` |
| **Indiana Secretary of State Business Registry** | N. Data quality / verification sources / Open Data Portal | [indiana.gov](https://www.indiana.gov/business) | `playwright` | `sources` |

## 2. Detailed Category Targets

### Category: A. State identity and geography (County List)
- **Source Name:** Indiana County Metadata
- **Source URL:** [https://www.indiana.gov](https://www.indiana.gov)
- **Domain:** `indiana.gov`
- **Target Table:** `counties`
- **Expected Fields:** `name, state_id`
- **Crawl Method:** `static_fetch`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Indiana.
- **Last Checked:** 2026-06-13

### Category: B. Medicaid / benefits / HHS (Medicaid Landing Page)
- **Source Name:** Indiana Medicaid Portal
- **Source URL:** [https://dhhs.indiana.gov](https://dhhs.indiana.gov)
- **Domain:** `dhhs.indiana.gov`
- **Target Table:** `county_offices`
- **Expected Fields:** `office_name, address, phone`
- **Crawl Method:** `playwright`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Indiana.
- **Last Checked:** 2026-06-13

### Category: C. Developmental disability / DD / IDD services (State DD Agency Page)
- **Source Name:** Indiana Developmental Services Directory
- **Source URL:** [https://dhhs.indiana.gov/dd](https://dhhs.indiana.gov/dd)
- **Domain:** `dhhs.indiana.gov`
- **Target Table:** `state_resource_agencies`
- **Expected Fields:** `name, phone, website`
- **Crawl Method:** `playwright`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Indiana.
- **Last Checked:** 2026-06-13

### Category: D. HCBS waivers (Waiver Page)
- **Source Name:** Indiana HCBS Waivers Page
- **Source URL:** [https://dhhs.indiana.gov/dd/waivers](https://dhhs.indiana.gov/dd/waivers)
- **Domain:** `dhhs.indiana.gov`
- **Target Table:** `programs`
- **Expected Fields:** `program_id, name`
- **Crawl Method:** `static_fetch`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Indiana.
- **Last Checked:** 2026-06-13

### Category: E. Early intervention (Early Intervention Landing Page)
- **Source Name:** Indiana Early Intervention / Part C
- **Source URL:** [https://dhhs.indiana.gov/earlyintervention](https://dhhs.indiana.gov/earlyintervention)
- **Domain:** `dhhs.indiana.gov`
- **Target Table:** `programs`
- **Expected Fields:** `name, intake_phone`
- **Crawl Method:** `static_fetch`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Indiana.
- **Last Checked:** 2026-06-13

### Category: F. Special education / IEP (SEA Special Ed Page)
- **Source Name:** Indiana Department of Education Special Ed
- **Source URL:** [https://education.indiana.gov](https://education.indiana.gov)
- **Domain:** `education.indiana.gov`
- **Target Table:** `programs`
- **Expected Fields:** `program_id, name`
- **Crawl Method:** `static_fetch`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Indiana.
- **Last Checked:** 2026-06-13

### Category: G. Regional education structures (Regional Agency Directory)
- **Source Name:** Indiana Regional Special Education Support
- **Source URL:** [https://education.indiana.gov/regional](https://education.indiana.gov/regional)
- **Domain:** `education.indiana.gov`
- **Target Table:** `regional_education_agencies`
- **Expected Fields:** `name, website`
- **Crawl Method:** `playwright`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Indiana.
- **Last Checked:** 2026-06-13

### Category: H. Parent training / disability rights / legal aid (PTI Center)
- **Source Name:** Indiana Parent Training Center
- **Source URL:** [https://www.parentcenterhub.org](https://www.parentcenterhub.org)
- **Domain:** `parentcenterhub.org`
- **Target Table:** `nonprofit_organizations`
- **Expected Fields:** `name, website, phone`
- **Crawl Method:** `static_fetch`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Indiana.
- **Last Checked:** 2026-06-13

### Category: I. Condition-specific nonprofits (The Arc State Chapter)
- **Source Name:** The Arc of Indiana
- **Source URL:** [https://www.thearcindiana.org](https://www.thearcindiana.org)
- **Domain:** `thearcindiana.org`
- **Target Table:** `nonprofit_organizations`
- **Expected Fields:** `name, website, phone`
- **Crawl Method:** `static_fetch`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Indiana.
- **Last Checked:** 2026-06-13

### Category: J. Provider and advocate directories (Attorney Directory)
- **Source Name:** Special Education Attorneys Directory
- **Source URL:** [https://www.copaa.org](https://www.copaa.org)
- **Domain:** `copaa.org`
- **Target Table:** `iep_advocates`
- **Expected Fields:** `name, phone, email`
- **Crawl Method:** `playwright`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Indiana.
- **Last Checked:** 2026-06-13

### Category: K. Forms and guides (Forms Library)
- **Source Name:** Indiana Medicaid & Special Education Forms
- **Source URL:** [https://dhhs.indiana.gov/forms](https://dhhs.indiana.gov/forms)
- **Domain:** `dhhs.indiana.gov`
- **Target Table:** `forms`
- **Expected Fields:** `slug, download_url`
- **Crawl Method:** `pdf_extract`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Indiana.
- **Last Checked:** 2026-06-13

### Category: L. Transition / adult services (Vocational Rehabilitation)
- **Source Name:** Indiana Vocational Rehabilitation Services
- **Source URL:** [https://dhhs.indiana.gov/rehab](https://dhhs.indiana.gov/rehab)
- **Domain:** `dhhs.indiana.gov`
- **Target Table:** `programs`
- **Expected Fields:** `name, website`
- **Crawl Method:** `static_fetch`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Indiana.
- **Last Checked:** 2026-06-13

### Category: M. Hospitals / university clinics (Hospitals)
- **Source Name:** Indiana Children's Hospital Clinics
- **Source URL:** [https://www.childrenshospital.org](https://www.childrenshospital.org)
- **Domain:** `childrenshospital.org`
- **Target Table:** `resource_providers`
- **Expected Fields:** `name, phone, address`
- **Crawl Method:** `manual_review`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Indiana.
- **Last Checked:** 2026-06-13

### Category: N. Data quality / verification sources (Open Data Portal)
- **Source Name:** Indiana Secretary of State Business Registry
- **Source URL:** [https://www.indiana.gov/business](https://www.indiana.gov/business)
- **Domain:** `indiana.gov`
- **Target Table:** `sources`
- **Expected Fields:** `url, type`
- **Crawl Method:** `playwright`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Indiana.
- **Last Checked:** 2026-06-13

