# State Source Targets: Kansas (KS)

This document outlines the authoritative crawler target domains, specific agencies, and extraction methods required to replace programmatic placeholders in Kansas with real, source-listed records.

> [!NOTE]
> **Category Scaffold Complete:** This state is currently mapped as a category-level scaffold (14 targets). Source-level expansion will follow in Wave 4.

## 1. Domain Crawler Targets (Wave 4)

| Target Name | Category / Subcategory | Source Domain | Crawl Method | Target Table |
| :--- | :--- | :--- | :--- | :--- |
| **Kansas County Metadata** | A. State identity and geography / County List | [kansas.gov](https://www.kansas.gov) | `static_fetch` | `counties` |
| **Kansas Medicaid Portal** | B. Medicaid / benefits / HHS / Medicaid Landing Page | [dhhs.kansas.gov](https://dhhs.kansas.gov) | `playwright` | `county_offices` |
| **Kansas Developmental Services Directory** | C. Developmental disability / DD / IDD services / State DD Agency Page | [dhhs.kansas.gov](https://dhhs.kansas.gov/dd) | `playwright` | `state_resource_agencies` |
| **Kansas HCBS Waivers Page** | D. HCBS waivers / Waiver Page | [dhhs.kansas.gov](https://dhhs.kansas.gov/dd/waivers) | `static_fetch` | `programs` |
| **Kansas Early Intervention / Part C** | E. Early intervention / Early Intervention Landing Page | [dhhs.kansas.gov](https://dhhs.kansas.gov/earlyintervention) | `static_fetch` | `programs` |
| **Kansas Department of Education Special Ed** | F. Special education / IEP / SEA Special Ed Page | [education.kansas.gov](https://education.kansas.gov) | `static_fetch` | `programs` |
| **Kansas Regional Special Education Support** | G. Regional education structures / Regional Agency Directory | [education.kansas.gov](https://education.kansas.gov/regional) | `playwright` | `regional_education_agencies` |
| **Kansas Parent Training Center** | H. Parent training / disability rights / legal aid / PTI Center | [parentcenterhub.org](https://www.parentcenterhub.org) | `static_fetch` | `nonprofit_organizations` |
| **The Arc of Kansas** | I. Condition-specific nonprofits / The Arc State Chapter | [thearckansas.org](https://www.thearckansas.org) | `static_fetch` | `nonprofit_organizations` |
| **Special Education Attorneys Directory** | J. Provider and advocate directories / Attorney Directory | [copaa.org](https://www.copaa.org) | `playwright` | `iep_advocates` |
| **Kansas Medicaid & Special Education Forms** | K. Forms and guides / Forms Library | [dhhs.kansas.gov](https://dhhs.kansas.gov/forms) | `pdf_extract` | `forms` |
| **Kansas Vocational Rehabilitation Services** | L. Transition / adult services / Vocational Rehabilitation | [dhhs.kansas.gov](https://dhhs.kansas.gov/rehab) | `static_fetch` | `programs` |
| **Kansas Children's Hospital Clinics** | M. Hospitals / university clinics / Hospitals | [childrenshospital.org](https://www.childrenshospital.org) | `manual_review` | `resource_providers` |
| **Kansas Secretary of State Business Registry** | N. Data quality / verification sources / Open Data Portal | [kansas.gov](https://www.kansas.gov/business) | `playwright` | `sources` |

## 2. Detailed Category Targets

### Category: A. State identity and geography (County List)
- **Source Name:** Kansas County Metadata
- **Source URL:** [https://www.kansas.gov](https://www.kansas.gov)
- **Domain:** `kansas.gov`
- **Target Table:** `counties`
- **Expected Fields:** `name, state_id`
- **Crawl Method:** `static_fetch`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Kansas.
- **Last Checked:** 2026-06-13

### Category: B. Medicaid / benefits / HHS (Medicaid Landing Page)
- **Source Name:** Kansas Medicaid Portal
- **Source URL:** [https://dhhs.kansas.gov](https://dhhs.kansas.gov)
- **Domain:** `dhhs.kansas.gov`
- **Target Table:** `county_offices`
- **Expected Fields:** `office_name, address, phone`
- **Crawl Method:** `playwright`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Kansas.
- **Last Checked:** 2026-06-13

### Category: C. Developmental disability / DD / IDD services (State DD Agency Page)
- **Source Name:** Kansas Developmental Services Directory
- **Source URL:** [https://dhhs.kansas.gov/dd](https://dhhs.kansas.gov/dd)
- **Domain:** `dhhs.kansas.gov`
- **Target Table:** `state_resource_agencies`
- **Expected Fields:** `name, phone, website`
- **Crawl Method:** `playwright`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Kansas.
- **Last Checked:** 2026-06-13

### Category: D. HCBS waivers (Waiver Page)
- **Source Name:** Kansas HCBS Waivers Page
- **Source URL:** [https://dhhs.kansas.gov/dd/waivers](https://dhhs.kansas.gov/dd/waivers)
- **Domain:** `dhhs.kansas.gov`
- **Target Table:** `programs`
- **Expected Fields:** `program_id, name`
- **Crawl Method:** `static_fetch`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Kansas.
- **Last Checked:** 2026-06-13

### Category: E. Early intervention (Early Intervention Landing Page)
- **Source Name:** Kansas Early Intervention / Part C
- **Source URL:** [https://dhhs.kansas.gov/earlyintervention](https://dhhs.kansas.gov/earlyintervention)
- **Domain:** `dhhs.kansas.gov`
- **Target Table:** `programs`
- **Expected Fields:** `name, intake_phone`
- **Crawl Method:** `static_fetch`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Kansas.
- **Last Checked:** 2026-06-13

### Category: F. Special education / IEP (SEA Special Ed Page)
- **Source Name:** Kansas Department of Education Special Ed
- **Source URL:** [https://education.kansas.gov](https://education.kansas.gov)
- **Domain:** `education.kansas.gov`
- **Target Table:** `programs`
- **Expected Fields:** `program_id, name`
- **Crawl Method:** `static_fetch`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Kansas.
- **Last Checked:** 2026-06-13

### Category: G. Regional education structures (Regional Agency Directory)
- **Source Name:** Kansas Regional Special Education Support
- **Source URL:** [https://education.kansas.gov/regional](https://education.kansas.gov/regional)
- **Domain:** `education.kansas.gov`
- **Target Table:** `regional_education_agencies`
- **Expected Fields:** `name, website`
- **Crawl Method:** `playwright`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Kansas.
- **Last Checked:** 2026-06-13

### Category: H. Parent training / disability rights / legal aid (PTI Center)
- **Source Name:** Kansas Parent Training Center
- **Source URL:** [https://www.parentcenterhub.org](https://www.parentcenterhub.org)
- **Domain:** `parentcenterhub.org`
- **Target Table:** `nonprofit_organizations`
- **Expected Fields:** `name, website, phone`
- **Crawl Method:** `static_fetch`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Kansas.
- **Last Checked:** 2026-06-13

### Category: I. Condition-specific nonprofits (The Arc State Chapter)
- **Source Name:** The Arc of Kansas
- **Source URL:** [https://www.thearckansas.org](https://www.thearckansas.org)
- **Domain:** `thearckansas.org`
- **Target Table:** `nonprofit_organizations`
- **Expected Fields:** `name, website, phone`
- **Crawl Method:** `static_fetch`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Kansas.
- **Last Checked:** 2026-06-13

### Category: J. Provider and advocate directories (Attorney Directory)
- **Source Name:** Special Education Attorneys Directory
- **Source URL:** [https://www.copaa.org](https://www.copaa.org)
- **Domain:** `copaa.org`
- **Target Table:** `iep_advocates`
- **Expected Fields:** `name, phone, email`
- **Crawl Method:** `playwright`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Kansas.
- **Last Checked:** 2026-06-13

### Category: K. Forms and guides (Forms Library)
- **Source Name:** Kansas Medicaid & Special Education Forms
- **Source URL:** [https://dhhs.kansas.gov/forms](https://dhhs.kansas.gov/forms)
- **Domain:** `dhhs.kansas.gov`
- **Target Table:** `forms`
- **Expected Fields:** `slug, download_url`
- **Crawl Method:** `pdf_extract`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Kansas.
- **Last Checked:** 2026-06-13

### Category: L. Transition / adult services (Vocational Rehabilitation)
- **Source Name:** Kansas Vocational Rehabilitation Services
- **Source URL:** [https://dhhs.kansas.gov/rehab](https://dhhs.kansas.gov/rehab)
- **Domain:** `dhhs.kansas.gov`
- **Target Table:** `programs`
- **Expected Fields:** `name, website`
- **Crawl Method:** `static_fetch`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Kansas.
- **Last Checked:** 2026-06-13

### Category: M. Hospitals / university clinics (Hospitals)
- **Source Name:** Kansas Children's Hospital Clinics
- **Source URL:** [https://www.childrenshospital.org](https://www.childrenshospital.org)
- **Domain:** `childrenshospital.org`
- **Target Table:** `resource_providers`
- **Expected Fields:** `name, phone, address`
- **Crawl Method:** `manual_review`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Kansas.
- **Last Checked:** 2026-06-13

### Category: N. Data quality / verification sources (Open Data Portal)
- **Source Name:** Kansas Secretary of State Business Registry
- **Source URL:** [https://www.kansas.gov/business](https://www.kansas.gov/business)
- **Domain:** `kansas.gov`
- **Target Table:** `sources`
- **Expected Fields:** `url, type`
- **Crawl Method:** `playwright`
- **Robots.txt Status:** `allowed`
- **Terms Risk:** `low`
- **Priority:** 2
- **Notes:** Initial category-level scaffold source target for Kansas.
- **Last Checked:** 2026-06-13

