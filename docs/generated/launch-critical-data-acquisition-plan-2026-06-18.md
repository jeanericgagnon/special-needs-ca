...
- Artifact target: `docs/generated/launch-critical-data-acquisition-plan-2026-06-18.md`
...
- Queue baseline: readyRows=703, ready_lightweight=308, ready_js_heavy=392, ready_pdf=1, discovery_only=2, missingSourceFamilyCount=0

## Authoritative Inputs

- completionPlanPath: `docs/generated/source-acquisition-completion-plan-2026-06-18.json`
- fullInformationGapAuditPath: `docs/generated/full-information-gap-audit-2026-06-18.json`
- missingSourceFamiliesPath: `docs/generated/missing-source-families-2026-06-18.json`
- informationInventoryPath: `docs/generated/information-inventory-2026-06-18.json`
- scrapeNowOnlyPath: `docs/generated/scrape-now-only-2026-06-18.json`
- providerSourcePackPath: `docs/generated/provider-source-pack-plan-2026-06-18.json`
- knowledgeStatusQueuePath: `docs/generated/knowledge-content-status-queue-2026-06-18.json`
...
- authoredMissingSourceTargetsPath: `docs/generated/authored-missing-source-targets-2026-06-18.json`
...
- followupReadyRows: 15
- followupReadyStates: 11
...
- Queue counts:
- total: 29
- byStatus: ready_lightweight=29
- byQueueSource: db_discovered_ready=21, master_ledger_ready=8
...
Gap analysis
- Missing pieces:
- Burn down the remaining 29 ready rows.
...
- Completion plan already has 29 ready lightweight rows.
...
- Queue counts:
- total: 11
- byStatus: ready_lightweight=11
- byQueueSource: master_ledger_ready=11
- Trust and composition:
...
- Existing source-pack status:
- Completion plan has 11 ready lightweight waiver rows.
...
- Queue counts:
- total: 116
- byStatus: ready_js_heavy=9, ready_lightweight=106, ready_pdf=1
- byQueueSource: db_discovered_ready=106, master_ledger_ready=10
- Trust and composition:
...
- 116 queue rows remain, with most still tied to malformed or challenge-blocked targets.
- Authoring needed first: yes
...
- Staging counts:
- stagingScrapedStateResourceAgencies: 448
- Queue counts:
- total: 74
- byStatus: ready_js_heavy=47, ready_lightweight=27
- byQueueSource: db_discovered_ready=24, master_ledger_ready=50
- Trust and composition:
...
- Staging counts:
- stagingScrapedRegionalEducationAgencies: 161
- stagingScrapedSchoolDistricts: 2874
- Queue counts:
- total: 11
- byStatus: ready_js_heavy=10, ready_lightweight=1
- byQueueSource: master_ledger_ready=11
- Trust and composition:
...
Gap analysis
- Missing pieces:
- Process the 11 remaining queue rows.
...
Current-state inventory
- Live counts:
- resourceProviders: 94
- Staging counts:
- stagingScrapedResourceProviders: 120
- Queue counts:
- total: 1
- byStatus: discovery_only=1
- byQueueSource: authored_missing_family=1
- Trust and composition:
- verificationStatus: official_verified=6, source_listed=88
- claimStatus: null=11, unclaimed=83
...
- anchorsRequiredPerState: 3
- currentStatus: launchReadyStates=0, blockedStates=0, notReadyAuthorFirstStates=50, pullNowPlannedStates=10, authoredProviderTargets=2, directReadyQueueRows=0, discoveryOnlyRows=1
...
- Authored targets for providers_care remain only 2.
- Authoring needed first: yes
...
- Queue counts:
- trackedTargets: 65
- finalStatus: deferred_blocked_source=52, deferred_unresolved=5, duplicate_of_existing_live_article=6, promoted_live=2
- Trust and composition:
- authoredTargetsForFamily: 68
...
- Existing source-pack status:
- 68 authored knowledge-content targets exist.
- 2 are promoted live, 52 are blocked, and 5 remain unresolved.
- Authoring needed first: yes
...
- programs_benefits: queue=29; lane=HTTP; batch=large; next=Run deterministic reject suppression for missing_action_signal and missing_program_name before each scrape wave.
- waivers: queue=11; lane=HTTP first, PDF if new repaired rows surface; batch=large HTTP, medium PDF; next=Do not treat generic policy pages as waivers unless program identity and action path are explicit.
- dd_routing: queue=74; lane=HTTP for lightweight rows, Playwright for JS-heavy rows; batch=large HTTP, small JS-heavy; next=Closure requires both sublanes to complete or block explicitly; do not report DD routing done after HTTP only.
- education_routing: queue=11; lane=HTTP for lightweight rows, Playwright for JS-heavy rows; batch=large HTTP, small JS-heavy; next=Regional routing coverage should close first; district-only fallback is additive, not a substitute for missing state/regional routing.

### Author first

- forms_guides: queue=0; lane=HTTP primarily, PDF where exact form downloads are the only stable source; batch=large HTTP, medium PDF; next=Do not requeue fallback-only states as ready until each state has a final official source class.
- program_waitlists: queue=6; lane=HTTP; batch=large; next=Surface program_waitlists as a first-class launch family before bulk execution so closure can be measured directly.
- medicaid_hhs_offices: queue=116; lane=HTTP after repair, JS or PDF only for residual exceptions; batch=large after repair, medium or small for residual JS/PDF; next=Do not rerun county-office bulk scraping until repair-pack replacements resolve malformed county-domain families.
- providers_care: queue=1; lane=HTTP where static; Playwright for JS-heavy clinic systems; batch=small; next=Do not run more provider waves until state anchor packs convert pull-now states into exact ready targets.
- knowledge_content: queue=0; lane=HTTP first, JS only for reviewed replacements that truly require it; batch=small; next=Replace blocked targets before any new fetch wave and keep knowledge rows out of local directory targets.
...
| family | current count/status | launch threshold | current % to threshold | blocking gap type | next control-plane artifact | next command |
|---|---|---|---|---|---|---|
| programs_benefits | 29 ready rows unresolved; 507 live programs | 29/29 ready rows resolved | 0% queue closure | validator/truth-policy | source-acquisition-completion-plan | npm run audit:source-acquisition-completion-plan |
| waivers | 11 ready rows unresolved; 8 typed live waiver rows | 50/50 states have at least one explicit waiver path or explicit no-waiver/blocked state artifact | 16% state coverage / 0% queue closure | queue/promotion | source-acquisition-completion-plan | npm run audit:source-acquisition-completion-plan |
| forms_guides | 7 cleared states; 6 ready exact; 37 author-first fallback | 50/50 states classified and placeholder-free | 14% cleared / 100% accounted | source/authoring | forms_source_pack | npm run audit:forms-source-pack |
| program_waitlists | 6 exact curated launch rows; 15 followup ready rows hidden under general_gap_fill | 6/6 curated exact waitlist targets classified as first-class launch rows | 100% identified / 0% first-class closed | queue/accounting | launch-critical-data-plan | npm run audit:launch-critical-data-plan |
| medicaid_hhs_offices | 50/50 state coverage live; 116 remaining rows need repair-vs-ready classification | 50/50 states truthful + 116/116 rows reclassified | 100% state coverage / 0% repair closure | source/queue | official_state_domain_repairs | npm run audit:source-acquisition-completion-plan |
| dd_routing | 50/50 state coverage live; 74 ready rows unresolved | 50/50 states truthful + 74/74 rows resolved or blocked | 100% state coverage / 0% queue closure | queue/scraper | source-acquisition-completion-plan | npm run audit:source-acquisition-completion-plan |
| education_routing | 47/50 regional state coverage; 11 ready rows unresolved | 50/50 states with regional or district fallback path | 94% coverage / 0% queue closure | queue/source | source-acquisition-completion-plan | npm run audit:source-acquisition-completion-plan |
| providers_care | 0 launch-ready states; 10 pull-now planned; 2 authored targets; 1 discovery-only row | 50/50 states decision-complete under the provider launch standard | 0% launch-ready / 20% author-planned | source/queue | provider-source-pack-plan | npm run audit:provider-source-pack |
| knowledge_content | 2 promoted live; 52 blocked; 5 unresolved; provenance not yet launch-safe | 6/6 topic buckets with provenance-safe serving path | 0% launch-safe topic closure | promotion/truth-policy | knowledge-content-status-queue | npm run audit:knowledge-content-status-queue |
| disability_to_program_matching | reference tables present; no direct queue | dependency verification only after linked-family closure | 100% direct data present / 0% dependency verification complete | dependency/verification | launch-critical-data-plan | npm run audit:launch-critical-data-plan |
