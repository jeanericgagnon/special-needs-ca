...
- completeness_pct: 83
- county_count: 33
- primary_gap_reason: official_webed_directory_workbooks_are_live_single_sheet_exports_with_no_county_field_or_rec_service_area_contract_and_official_dvr_root_still_returns_401_without_reviewed_public_alternate
...
- district_or_county_education_routing: blocked_official_directory_live_but_verified_workbooks_still_lack_county_crosswalk (Reviewed 2026-06-25 one bounded official New Mexico education directory pass beyond the previously failing PED hosts, then rechecked the live PED-managed workbook exports directly on 2026-06-25. The public SharePoint school-directory host `https://webed.ped.state.nm.us/sites/schooldirectory/SitePages/Home.aspx` is live and returns HTTP 200, and the official public downloads `NM Schools.xlsx`, `Superintendents.xlsx`, and `REC Directors.xlsx` also return HTTP 200 from the same host. A fresh bounded workbook-structure inspection confirmed all three are single-sheet exports: `NM Schools`, `Superintendents`, and `REC Directors`. `NM Schools.xlsx` exposes district and school contact columns but no county field. `Superintendents.xlsx` exposes district contact columns but no county field. `REC Directors.xlsx` exposes REC identity and contact columns but no county-service-area labels. The legacy `education.new-mexico.gov` host family remains DNS-dead, and the current `webnew.ped.state.nm.us/bureaus/special-education/` page still does not itself supply county-grade local routing. New Mexico education therefore remains blocked on an official county-to-district or county-to-REC crosswalk, not on total absence of a public PED directory lane.)
...
- district_or_county_education_routing: official_webed_directory_workbooks_verified_no_county_field_or_rec_service_area_contract :: Reviewed 2026-06-25 one bounded official New Mexico education directory pass beyond the previously failing PED hosts, then rechecked the live PED-managed workbook exports directly on 2026-06-25. The public SharePoint school-directory host `https://webed.ped.state.nm.us/sites/schooldirectory/SitePages/Home.aspx` is live and returns HTTP 200, and the official public downloads `NM Schools.xlsx`, `Superintendents.xlsx`, and `REC Directors.xlsx` also return HTTP 200 from the same host. A fresh bounded workbook-structure inspection confirmed all three are single-sheet exports with district, superintendent, and REC contact columns but no county field and no REC county-service-area labels. New Mexico education therefore remains blocked on an official county-to-district or county-to-REC crosswalk, not on total absence of a public PED directory lane.
...
- district_or_county_education_routing: blocked_official_directory_live_but_verified_workbooks_still_lack_county_crosswalk; samples=5; first=https://webed.ped.state.nm.us/sites/schooldirectory/SitePages/Home.aspx
