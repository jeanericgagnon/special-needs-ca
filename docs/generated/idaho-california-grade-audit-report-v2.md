...
- completeness_pct: 85
- county_count: 44
- primary_gap_reason: remaining_idaho_district_roots_and_live_sitemaps_still_materialize_only_contact_title_ix_or_generic_leaves_without_special_education_routing
...
- district_or_county_education_routing: blocked_remaining_live_district_roots_and_sitemaps_only_materialize_contact_title_ix_or_generic_leaves_after_jefferson_and_oneida_recovery (Reviewed 2026-06-25 one more bounded official Idaho district pass focused on the exact same-host leaves and current machine-readable inventories for the four remaining district roots. Camas still only materializes a district-owned `Contact Information` leaf with district address and phone, and both `robots.txt` and `sitemap.xml` 404; the apparent raw `504` token on Camas resolves only to numeric payload noise rather than a role-bearing Section 504 leaf. Clark now clearly materializes exact district-owned `Contact Us` and `Title IX` leaves and also publishes a real XML sitemap at `/fs/pages/sitemap`, but that inventory still yields no special-education, special-services, student-services, section-504, child-find, or procedural-safeguards page. Fremont now clearly materializes exact district-owned `Contact Us` and `Title IX-Sexual Harassment` leaves and also publishes a live sitemap at `/sitemap.xml`; additional bounded checks on `/documents` and `/page/district-forms` stayed generic, and direct bounded probes for special-education, special-services, student-services, 504, and child-find all 404. Shoshone remains live and now has its post sitemaps rechecked, but the page/post feeds still only materialize generic district, board, agenda, and newsletter pages and the taxonomy sitemap guess 404s; direct bounded probes for special-education, student-services, special-services, section-504, and child-find all 404. Idaho therefore remains blocked on a smaller but better-defined education remainder: the surviving district-owned roots, documents lanes, and sitemaps are real, but they still materialize only contact, Title IX, or generic leaves rather than local special-education routing.)
...
- district_or_county_education_routing: blocked_remaining_live_district_roots_only_materialize_contact_title_ix_or_generic_leaves_after_jefferson_and_oneida_recovery; samples=37; first=https://5il.co/26a73
