...
- district_or_county_education_routing: blocked_official_district_directory_without_county_or_special_education_fields (Reviewed 2026-06-22 current official Idaho SDE pages plus the missed official School Districts directory at https://www.sde.idaho.gov/school-districts/. The state page now preserves 116 exact district website links from one first-party directory, so the old "no exact leaves" claim is no longer true. Idaho still remains blocked at county-grade education routing because the current school_district DB rows are still 44/44 statewide or generic fallbacks: 42 rows use `https://www.sde.idaho.gov/sped/` and the remaining 2 rows (Ada and Canyon) use the generic SDE root `https://www.sde.idaho.gov/`, while bounded live content checks show the public directory page does not expose county-keyed mapping or district special-education contacts.)
...
- county_local_disability_resources: blocked_exact_office_leafs_exist_but_public_directory_lacks_county_terms (Reviewed 2026-06-22 current official Idaho DHW office routing pages plus the paginated office directory. The official directory still exposes 27 exact office leaves, but a live DB reconciliation shows 18 current county_offices rows still point to DOI mirror sources, 27 rows still rely on the dead legacy `dhhs.idaho.gov/locations` root, and Canyon County is duplicated across two DOI-backed office rows (Caldwell Office and Nampa Office). Bounded live content checks show the public directory page does not expose county labels such as Canyon County, and the only public Nampa mention is Southwest Idaho Treatment Center (SWITC), not a county office or benefits office leaf. The public DHW office stack remains city-or-ZIP search only, so county-to-office routing is still not publicly verifiable.)
...
- County-local is narrower: the missing Nampa/Canyon office question is now resolved negatively, because the official directory mentions Nampa only as Southwest Idaho Treatment Center rather than as a county office leaf, and the live DB now proves Canyon County is duplicated across two DOI-backed office rows instead of having one reviewed county-keyed replacement.
